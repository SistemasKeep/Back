...
async function index(req, res) {
	...
	const camposModelo = Object.keys(db.sequelize.models.polizas_modalidades.rawAttributes);
	...
	try {
		const perfilesValidos = ['poliza_detalle', 'modalidad_transporte', 'all']
		var relaciones = []
		...
		const dataDocs = await db.sequelize.models.polizas_modalidades.count({
		    paranoid: false,
		    include: relaciones,
		    where: filtro
		});
		...
		const fullUrl = `${req.protocol}://${req.get('host')}/api/polizasModalidades`;
...
async function store(req, res){
	try {
		...
		const registrosEncontrados = await db.sequelize.models.polizas_modalidades.findAll({
			where: {
		...
		if(registrosEncontrados.length > 0){
			...
			await registrosEncontrados.forEach(registro => {
				if((registro.id_poliza_detalle == parametros.idPolizaDetalle) &&
                   registro.id_modalidad == parametros.idModalidad){
					if(!regExistente){
						regExistente = true;
		...
		const nuevoRegistro = await db.sequelize.models.polizas_modalidades.create(registro);
...
async function show(req, res){
	...
	try {
		...
		const registroEncontrado = await db.sequelize.models.polizas_modalidades.findByPk(id, {include:relaciones,paranoid: false});
...
async function update(req, res){
	...
	try {
		...
		const registroAEditar = await db.sequelize.models.polizas_modalidades.findByPk(id);
		...
		const registrosEncontrados = await db.sequelize.models.polizas_modalidades.findAll(whereFind);
		if(registrosEncontrados.length > 0){
			var regExistente = false
			await registrosEncontrados.forEach(registro => {
				if(((registro.id_poliza_detalle == (parametros.idPolizaDetalle != undefined ? parametros.idPolizaDetalle : registroAEditar.id_poliza_detalle)) &&
                   registro.id_modalidad == (parametros.idModalidad != undefined ? parametros.idModalidad : registroAEditar.id_modalidad)) &&
				   registro.id != id){
					if(!regExistente){
						regExistente = true;
						res.status(400).send({ status: false, msg: "Registro existente"});
		...
		var registro2 = {
			...
			tabla: db.sequelize.models.polizas_modalidades.name.toUpperCase(),
			accion: 'EDICION',
			createdAt: moment().tz('America/Mexico_City')
		}
...
async function destroy(req,res){
	...
	try {
		const registroAEliminar = await db.sequelize.models.polizas_modalidades.findByPk(id);
		if(registroAEliminar != null){
			...
			for (const modelo of Object.values(db.sequelize.models)) {
				...
				for (const asociacion of Object.values(asociaciones)) {
					if(asociacion.target.name == db.sequelize.models.polizas_modalidades.name){
						let where = {}
...
async function restaurar(req,res){
	...
	try {
		const registroARestaurar = await db.sequelize.models.polizas_modalidades.findByPk(id,{ paranoid: false });
				...
				if(registrosEncontrados.length > 0){
					...
					await registrosEncontrados.forEach(registro => {
						if(((registro.id_poliza_detalle == registroARestaurar.id_poliza_detalle) ||
						   registro.id_modalidad == registroARestaurar.id_modalidad) &&
						   registro.id != id){
							if(!regExistente){
								regExistente = true;
								res.status(400).send({ status: false, msg: "Registro existente"});
...
async function showHistoricos(req, res) {
	...
	if(registro.tabla != db.sequelize.models.polizas_modalidades.name.toUpperCase()){
		return res.status(400).send({ status: false, msg: "Registro no pertenece al crud polizas_modalidades" });
	} 
	let usuario = await db.sequelize.models.usuarios.findByPk(registro.id_usuario_registro);
	reg.id = registro.id;
	reg.usuario_registro = {id: usuario.id, nombre: usuario.nombre};
