...
async function index(req, res) {
	...
	const camposModelo = Object.keys(db.sequelize.models.config_smtp_marcas.rawAttributes);
	...
	try {
		...
		const docs = await db.sequelize.models.config_smtp_marcas.findAll({
			paranoid: false,
			page: page || 1,
			include: relaciones,
		...
		const dataDocs = await db.sequelize.models.config_smtp_marcas.count({
			paranoid: false,
			include: relaciones,
			where: filtro
		});
		...
		const fullUrl = `${req.protocol}://${req.get('host')}/api/configSmtpMarcas`;
...
async function store(req, res){
	try {
		...
		const registrosEncontrados = await db.sequelize.models.config_smtp_marcas.findAll({
			where: {
				[db.Sequelize.Op.and]: {
		...
		if(registrosEncontrados.length > 0){
			...
			await registrosEncontrados.forEach(registro => {
				if(registro.mail_username == parametros.mailUsername.toUpperCase() && registro.id_marca == parametros.idMarca){
					regExistente = true;
		...
		const nuevoRegistro = await db.sequelize.models.config_smtp_marcas.create(registro);
...
async function show(req, res){
	...
	try {
		...
		const registroEncontrado = await db.sequelize.models.config_smtp_marcas.findByPk(id,{include:relaciones,paranoid: false});
...
async function update(req, res){
	...
	try {
		...
		const registroAEditar = await db.sequelize.models.config_smtp_marcas.findByPk(id);
		...
		const registrosEncontrados = await db.sequelize.models.config_smtp_marcas.findAll(whereFind);
		if(registrosEncontrados.length > 0){
			var regExistente = false
			await registrosEncontrados.forEach(registro => {
                const idMarca = parametros.idMarca != undefined ? parametros.idMarca : registroAEditar.id_marca;
                const mailUsername = parametros.mailUsername != undefined ? parametros.mailUsername.toUpperCase() : registroAEditar.mail_username;
				if(registro.id_marca == idMarca && registro.mail_username == mailUsername && registro.id != id){
						regExistente = true;
						res.status(400).send({ status: false, msg: "Registro existente"});
			        }
			});
...
async function destroy(req,res){
	...
	try {
		const registroAEliminar = await db.sequelize.models.config_smtp_marcas.findByPk(id);
		if(registroAEliminar != null){
			...
			for (const modelo of Object.values(db.sequelize.models)) {
				...
				for (const asociacion of Object.values(asociaciones)) {
					if(asociacion.target.name == db.sequelize.models.config_smtp_marcas.name){
						let where = {}
...
async function restaurar(req,res){
	...
	try {
		const registroARestaurar = await db.sequelize.models.config_smtp_marcas.findByPk(id,{ paranoid: false });
		if(registroARestaurar != null){
			if(registroARestaurar.deletedAt != null){
				...
				if(registrosEncontrados.length > 0){
					var regExistente = false
					await registrosEncontrados.forEach(registro => {
						if((registro.id_marca == registroARestaurar.id_marca || registro.mail_username == registroARestaurar.mail_username) && registro.id != id){
							regExistente = true;
							res.status(400).send({ status: false, msg: "Registro existente"});
						}
					});
