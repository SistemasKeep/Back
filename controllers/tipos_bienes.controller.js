...
async function index(req, res) {
	...
	const camposModelo = Object.keys(db.sequelize.models.tipos_bienes.rawAttributes);
	...
	try {
		...
		const dataDocs = await db.sequelize.models.tipos_bienes.count({
			paranoid: false,
			include: relaciones,
			where: filtro
		});
		...
		const fullUrl = `${req.protocol}://${req.get('host')}/api/tiposBienes`;
...
async function store(req, res){
	...
	try {
		...
		const registrosEncontrados = await db.sequelize.models.tipos_bienes.findAll({
			where: {
				descripcion: {
		...
		const nuevoRegistro = await db.sequelize.models.tipos_bienes.create(registro);
...
async function show(req, res){
	...
	try {
		const findRelaciones = new Relaciones([],[],db.sequelize.models)
		const relaciones = await findRelaciones.getRelaciones()
		const registroEncontrado = await db.sequelize.models.tipos_bienes.findByPk(id,{ paranoid: false,include: relaciones, });
...
async function update(req, res){
	...
	try {
		...
		const registroAEditar = await db.sequelize.models.tipos_bienes.findByPk(id);
		...
		const registrosEncontrados = await db.sequelize.models.tipos_bienes.findAll(whereFind);
		if(registrosEncontrados.length > 0){
			var regExistente = false
			await registrosEncontrados.forEach(registro => {
		...
		var registro2 = {
			...
			tabla: db.sequelize.models.tipos_bienes.name.toUpperCase(),
			accion: 'EDICION',
			createdAt: moment().tz('America/Mexico_City')
		}
...
async function destroy(req,res){
	...
	try {
		const registroAEliminar = await db.sequelize.models.tipos_bienes.findByPk(id);
		if(registroAEliminar != null){
			...
			for (const modelo of Object.values(db.sequelize.models)) {
				...
				for (const asociacion of Object.values(asociaciones)) {
					if(asociacion.target.name == db.sequelize.models.tipos_bienes.name){
						let where = {}
...
async function restaurar(req,res){
	...
	try {
		const registroARestaurar = await db.sequelize.models.tipos_bienes.findByPk(id,{ paranoid: false });
...
async function showHistoricos(req, res) {
	...
	if(registro.tabla != db.sequelize.models.tipos_bienes.name.toUpperCase()){
		return res.status(400).send({ status: false, msg: "Registro no pertenece al crud tipos_bienes" });
	} 
	let usuario = await db.sequelize.models.usuarios.findByPk(registro.id_usuario_registro);
	reg.id = registro.id;
	reg.usuario_registro = {id: usuario.id, nombre: usuario.nombre};
