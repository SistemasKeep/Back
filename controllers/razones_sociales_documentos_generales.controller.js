...
async function index(req, res) {
	...
	const camposModelo = Object.keys(db.sequelize.models.razones_sociales_documentos_generales.rawAttributes);
	...
	try {
		...
		const docs = await db.sequelize.models.razones_sociales_documentos_generales.findAll({
			paranoid: false,
			page: page || 1,
			include: relaciones,
		...
		const dataDocs = await db.sequelize.models.razones_sociales_documentos_generales.count({
		    paranoid: false,
		    include: relaciones,
		    where: filtro
		});
		...
		const fullUrl = `${req.protocol}://${req.get('host')}/api/razonesSocialesDocumentosGenerales`;
...
async function store(req, res){
	try {
		...
		const registrosEncontrados = await db.sequelize.models.razones_sociales_documentos_generales.findAll({
			where: {
				descripcion:{
					[db.Sequelize.Op.like]: `%${parametros.descripcion}%`
				},
				id_marca: parametros.idMarca === undefined ? null : parametros.idMarca,
				deletedAt: null
			}
		});
		...
		const nuevoRegistro = await db.sequelize.models.razones_sociales_documentos_generales.create(registro);
...
async function show(req, res){
	...
	try {
	   const findRelaciones = new Relaciones([],[],db.sequelize.models)
		...
		const registroEncontrado = await db.sequelize.models.razones_sociales_documentos_generales.findByPk(id, {include:relaciones,paranoid: false});
...
async function update(req, res){
	...
	try {
		...
		const registroAEditar = await db.sequelize.models.razones_sociales_documentos_generales.findByPk(id);
		...
		var whereFind = {
			where: {
				...
				id_marca: parametros.idMarcar === undefined ? null : parametros.idMarca,
				deletedAt: null
			}
		}
		const registrosEncontrados = await db.sequelize.models.razones_sociales_documentos_generales.findAll(whereFind);
		if(registrosEncontrados.length > 0){
			var regExistente = false
			await registrosEncontrados.forEach(registro => {
		...
		if(datosUpdate.obligatorio === true || datosUpdate.obligatorio === false){
			console.log(datosUpdate.obligatorio)
			const razonesSocialesArchivos = await db.sequelize.models.razones_sociales_archivos.findAll({
				where:{
					id_documento_razon_social: id
				}
			});
			for(const razonSocialArchivo of razonesSocialesArchivos){
				await razonSocialArchivo.update({
					obligatorio:datosUpdate.obligatorio
				}, { where: { id: razonSocialArchivo.id } });
			}
		}
...
async function destroy(req,res){
	...
	try {
		const registroAEliminar = await db.sequelize.models.razones_sociales_documentos_generales.findByPk(id);
		if(registroAEliminar != null){
			...
			for (const modelo of Object.values(db.sequelize.models)) {
				...
				for (const asociacion of Object.values(asociaciones)) {
					if(asociacion.target.name == db.sequelize.models.razones_sociales_documentos_generales.name){
						let where = {}
...
async function restaurar(req,res){
	...
	try {
		const registroARestaurar = await db.sequelize.models.razones_sociales_documentos_generales.findByPk(id,{ paranoid: false });
			...
			if(registroARestaurar.deletedAt != null){
				...
				if(registrosEncontrados.length > 0){
					...
					await registrosEncontrados.forEach(registro => {
						...
						registro.id_marca == (parametros.id_marca) &&   
						registro.id != id){
							if(!regExistente){
								regExistente = true;
								res.status(400).send({ status: false, msg: "Registro existente"});
