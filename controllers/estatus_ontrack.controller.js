...
async function index(req, res) {
	...
	const camposModelo = Object.keys(db.sequelize.models.estatus_ontrack.rawAttributes);
	if(!camposModelo.includes(campoOrden)){
		campoOrden = 'createdAt';
	}
	const filtro = await getFiltro(req.query,db.sequelize.models.estatus_ontrack);
	...
	try {
		...
		const docs = await db.sequelize.models.estatus_ontrack.findAll({
			paranoid: false,
			page: page || 1,
			include: relaciones,
		...
		const dataDocs = await db.sequelize.models.estatus_ontrack.count({
			paranoid: false,
			include: relaciones,
			where: filtro
		});
		...
		const fullUrl = `${req.protocol}://${req.get('host')}/api/estatusOnTrack`;
...
async function store(req, res){
	...
	try {
		...
		const registrosEncontrados = await db.sequelize.models.estatus_ontrack.findAll({
			where: {
                [db.Sequelize.Op.and]: {
                    descripcion: {
		...
		await db.sequelize.models.estatus_ontrack.create(registro);
...
async function show(req, res){
	...
	try {
		const findRel = new Relaciones([],[],db.sequelize.models)
		const relaciones = await findRel.getRelaciones()
		const registroEncontrado = await db.sequelize.models.estatus_ontrack.findByPk(id,{paranoid: false, include: relaciones});
...
async function update(req, res){
	...
	try {
		...
		const registroAEditar = await db.sequelize.models.estatus_ontrack.findByPk(id);
		...
		const registrosEncontrados = await db.sequelize.models.estatus_ontrack.findAll(whereFind);
		if(registrosEncontrados.length > 0){
...
async function destroy(req,res){
	...
	try {
		const registroAEliminar = await db.sequelize.models.estatus_ontrack.findByPk(id);
		if(registroAEliminar != null){
			...
			for (const modelo of Object.values(db.sequelize.models)) {
				...
				for (const asociacion of Object.values(asociaciones)) {
					if(asociacion.target.name == db.sequelize.models.estatus_ontrack.name){
						let where = {}
...
async function restaurar(req,res){
	...
	try {
		const registroARestaurar = await db.sequelize.models.estatus_ontrack.findByPk(id,{ paranoid: false });
		if(registroARestaurar != null){
			if(registroARestaurar.deletedAt != null){
				...
				if(registrosEncontrados.length > 0){
					let regExistente = false;
					await registrosEncontrados.forEach(registro => {
						if(registro.descripcion == registroARestaurar.descripcion && registro.id != id){
							if(!regExistente){
								regExistente = true;
								res.status(400).send({ status: false, msg: "Registro existente"});
							}
