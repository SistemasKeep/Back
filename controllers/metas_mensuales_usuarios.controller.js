...
async function index(req, res) {
	...
	const camposModelo = Object.keys(db.sequelize.models.metas_mensuales_usuarios.rawAttributes);
	...
	try {
		...
		if(perfilesValidos.includes(req.query.perfil)){
			...
			try {
				const relacionesValidas = [ 'usuario' ]
		...
		const docs = await db.sequelize.models.metas_mensuales_usuarios.findAll({
			paranoid: false,
			page: page || 1,
			include: relaciones,
		...
		const dataDocs = await db.sequelize.models.metas_mensuales_usuarios.count({
		    paranoid: false,
		    include: relaciones,
		    where: filtro
		});		
		...
		const fullUrl = `${req.protocol}://${req.get('host')}/api/metasMensaulesUsuarios`;
...
async function store(req, res){
	...
	try {
		...
		if(parametros.metaCitas == undefined){
			parametros.metaCitas = 0
		}
		if(parametros.metaClientesNuevos == undefined){
			parametros.metaClientesNuevos = 0
		}
		let obligatorios = [{campo:'idUsuario', tipo:'model',model:db.sequelize.models.usuarios},
							{campo:'montoUsd', tipo:'number'},
							{campo:'montoMxn', tipo:'number'},
							{campo:'mes', tipo:'number'},
							{campo:'anio', tipo:'number'},
							{campo:'metaCitas', tipo:'number'},
							{campo:'metaClientesNuevos', tipo:'number'},
		]
		...
		if(parametros.mes < 1 || parametros.mes > 12){
			return res.status(400).send({ status: false, msg: "Mes no válido."});
		}
		if(parametros.anio < 2022 || parametros.anio > 2100){
			return res.status(400).send({ status: false, msg: "Año no válido."});
		}
		const registrosEncontrados = await db.sequelize.models.metas_mensuales_usuarios.findAll({
			where: {
		...
		if(registrosEncontrados.length > 0){
			...
			await registrosEncontrados.forEach(registro => {
				if(registro.id_usuario == parametros.idUsuario &&
					registro.mes ==  parametros.mes &&
					registro.anio ==  parametros.anio){
						if(!regExistente){
							regExistente = true;
		...
		const nuevoRegistro = await db.sequelize.models.metas_mensuales_usuarios.create(registro);
...
async function show(req, res){
	...
	try {
		...
		const registroEncontrado = await db.sequelize.models.metas_mensuales_usuarios.findByPk(id,{include:relaciones,paranoid: false});
...
async function update(req, res){
	...
	try {
		...
		const validosOpcionales = [{campo:'montoUsd', tipo:'number'},
								   {campo:'montoMxn', tipo:'number'},
								   {campo:'mes', tipo:'number'},
								   {campo:'anio', tipo:'number'},
								   {campo:'metaCitas', tipo:'number'},
		...
		const registroAEditar = await db.sequelize.models.metas_mensuales_usuarios.findByPk(id);
		...
		var whereFind = {
			where: {
				...
				mes: parametros.mes != undefined ? parametros.mes : registroAEditar.mes,
				anio: parametros.anio != undefined ? parametros.anio : registroAEditar.anio,
				deletedAt: null
			}
		}
		const registrosEncontrados = await db.sequelize.models.metas_mensuales_usuarios.findAll(whereFind);
		if(registrosEncontrados.length > 0){
			var regExistente = false
			await registrosEncontrados.forEach(registro => {
				if(((registro.id_usuario == (parametros.idUsuario != undefined ? parametros.idUsuario : registroAEditar.id_usuario)) &&
				    (registro.mes == (parametros.mes != undefined ? parametros.mes : registroAEditar.mes)) &&
				    (registro.anio == (parametros.anio != undefined ? parametros.anio : registroAEditar.anio))) &&
				    registro.id != id){
						if(!regExistente){
							regExistente = true;
							res.status(400).send({ status: false, msg: "Registro existente"});
...
async function destroy(req,res){
	...
	try {
		const registroAEliminar = await db.sequelize.models.metas_mensuales_usuarios.findByPk(id);
		if(registroAEliminar != null){
			...
			for (const modelo of Object.values(db.sequelize.models)) {
				...
				for (const asociacion of Object.values(asociaciones)) {
					if(asociacion.target.name == db.sequelize.models.metas_mensuales_usuarios.name){
						let where = {}
...
async function restaurar(req,res){
	...
	try {
		const registroARestaurar = await db.sequelize.models.metas_mensuales_usuarios.findByPk(id,{ paranoid: false });
		if(registroARestaurar != null){
			if(registroARestaurar.deletedAt != null){
				const registrosEncontrados = await db.sequelize.models.metas_mensuales_usuarios.findAll({
					where: {
						id_usuario: registroARestaurar.id_usuario,
						mes: registroARestaurar.mes,
						anio: registroARestaurar.anio,
						deletedAt: null
					}
				});
				...
				if(registrosEncontrados.length > 0){
					var regExistente = false
					await registrosEncontrados.forEach(registro => {
						if((registro.id_usuario == registroARestaurar.id_usuario &&
							registro.mes == registroARestaurar.mes &&
							registro.anio == registroARestaurar.anio) &&
							registro.id != id){
								if(!regExistente){
									regExistente = true;
									res.status(400).send({ status: false, msg: "Registro existente"});
