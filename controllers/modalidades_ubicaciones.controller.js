...
async function index(req, res) {
	...
	const camposModelo = Object.keys(db.sequelize.models.modalidades_ubicaciones.rawAttributes);
	...
	try {
		const perfilesValidos = ['modalidad_transporte', 'ubicacion_bienes', 'all']
		var relaciones = []
		if(perfilesValidos.includes(req.query.perfil)){
			const parametrosRelaciones = {
				modalidad_transporte: [ 'modalidad_transporte' ],
				ubicacion_bienes: [ 'ubicacion_bienes' ],
				all: [ 'modalidad_transporte','ubicacion_bienes' ]
			}
			...
			try {
				const relacionesValidas = [ 'pais', 'pais.continente' ]
		...
		const dataDocs = await db.sequelize.models.modalidades_ubicaciones.count({
		    paranoid: false,
		    include: relaciones,
		    where: filtro
		});		
		...
		const fullUrl = `${req.protocol}://${req.get('host')}/api/modalidadesUbicaciones`;
...
async function store(req, res){
	...
	try {
		...
		let obligatorios = [{campo:'idModalidad', tipo:'model',model:db.sequelize.models.modalidades},
							{campo:'idUbicacionBienes', tipo:'model',model:db.sequelize.models.ubicaciones_bienes}]
		registro = await Validaciones.validParametros(req, res,obligatorios,registro);
		if(!registro){
			return '';
		}
		const registrosEncontrados = await db.sequelize.models.modalidades_ubicaciones.findAll({
			where: {
				id_modalidad: parametros.idModalidad,
				id_ubicacion_bienes: parametros.idUbicacionBienes,
				deletedAt: null
			}
		});
		if(registrosEncontrados.length > 0){
			...
			await registrosEncontrados.forEach(registro => {
				if(registro.id_modalidad == parametros.idModalidad &&
					registro.id_ubicacion_bienes ==  parametros.idUbicacionBienes){
						if(!regExistente){
							regExistente = true;
		...
		const nuevoRegistro = await db.sequelize.models.modalidades_ubicaciones.create(registro);
...
async function show(req, res){
	...
	try {
		...
		if(perfilesValidos.includes(req.query.perfil)){
			...
			try {
				const relacionesValidas = [ 'modalidad_transporte', 'ubicacion_bienes' ]
				const findRelaciones = new Relaciones(relacionesValidas,req.query.with.split(","),db.sequelize.models)
				relaciones = await findRelaciones.getRelaciones()
			} catch (error) {
				relaciones = []
		...
		const registroEncontrado = await db.sequelize.models.modalidades_ubicaciones.findByPk(id,{include:relaciones,paranoid: false});
...
async function update(req, res){
	...
	try {
		...
		const validosOpcionales = [{campo:'idModalidad', tipo:'model',model:db.sequelize.models.modalidades},
								   {campo:'idUbicacionBienes', tipo:'model',model:db.sequelize.models.ubicaciones_bienes}]
		...
		const registroAEditar = await db.sequelize.models.modalidades_ubicaciones.findByPk(id);
		...
		var whereFind = {
			where: {
				id_modalidad: (parametros.idModalidad != undefined ? parametros.idModalidad : registroAEditar.id_modalidad),
				id_ubicacion_bienes: (parametros.idUbicacionBienes != undefined ? parametros.idUbicacionBienes : registroAEditar.id_ubicacion_bienes),
				deletedAt: null
			}
		}
		const registrosEncontrados = await db.sequelize.models.modalidades_ubicaciones.findAll(whereFind);
		if(registrosEncontrados.length > 0){
			var regExistente = false
			await registrosEncontrados.forEach(registro => {
				if((registro.id_modalidad == (parametros.idModalidad != undefined ? parametros.idModalidad : registroAEditar.id_modalidad) &&
					registro.id_ubicacion_bienes ==  (parametros.idUbicacionBienes != undefined ? parametros.idUbicacionBienes : registroAEditar.id_ubicacion_bienes)) &&
					registro.id != id){
						if(!regExistente){
							regExistente = true;
							res.status(400).send({ status: false, msg: "Registro existente"});
		...
		var registro2 = {
			...
			tabla: db.sequelize.models.modalidades_ubicaciones.name.toUpperCase(),
			accion: 'EDICION',
			createdAt: moment().tz('America/Mexico_City')
		}
...
async function destroy(req,res){
	...
	try {
		const registroAEliminar = await db.sequelize.models.modalidades_ubicaciones.findByPk(id);
		if(registroAEliminar != null){
			...
			for (const modelo of Object.values(db.sequelize.models)) {
				...
				for (const asociacion of Object.values(asociaciones)) {
					if(asociacion.target.name == db.sequelize.models.modalidades_ubicaciones.name){
						let where = {}
...
async function restaurar(req,res){
	...
	try {
		const registroARestaurar = await db.sequelize.models.modalidades_ubicaciones.findByPk(id,{ paranoid: false });
		if(registroARestaurar != null){
			if(registroARestaurar.deletedAt != null){
				const registrosEncontrados = await db.sequelize.models.modalidades_ubicaciones.findAll({
					where: {
						id_modalidad: registroARestaurar.id_modalidad,
						id_ubicacion_bienes: registroARestaurar.id_ubicacion_bienes,
						deletedAt: null
					}
				});
				...
				if(registrosEncontrados.length > 0){
					var regExistente = false
					await registrosEncontrados.forEach(registro => {
						if(registro.id_modalidad == (registroARestaurar.id_modalidad) &&
							registro.id_ubicacion_bienes ==  (registroARestaurar.id_ubicacion_bienes)&&
							registro.id != id){
								if(!regExistente){
									regExistente = true;
									res.status(400).send({ status: false, msg: "Registro existente"});
...
async function showHistoricos(req, res) {
	...
	if(registro.tabla != db.sequelize.models.modalidades_ubicaciones.name.toUpperCase()){
		return res.status(400).send({ status: false, msg: "Registro no pertenece al crud modalidades_ubicaciones" });
	} 
	let usuario = await db.sequelize.models.usuarios.findByPk(registro.id_usuario_registro);
	reg.id = registro.id;
	reg.usuario_registro = {id: usuario.id, nombre: usuario.nombre};
