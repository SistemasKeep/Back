...
async function index(req, res) {
	...
	const camposModelo = Object.keys(db.sequelize.models.proveedores_documentos_generales.rawAttributes);
	...
	try {
		...
		const docs = await db.sequelize.models.proveedores_documentos_generales.findAll({
			paranoid: false,
			page: page || 1,
			include: relaciones,
		...
		const dataDocs = await db.sequelize.models.proveedores_documentos_generales.count({
		    paranoid: false,
		    include: relaciones,
		    where: filtro
		});
		...
		const fullUrl = `${req.protocol}://${req.get('host')}/api/proveedoresDocumentosGenerales`;
...
async function store(req, res){
	try {
		...
		const registrosEncontrados = await db.sequelize.models.proveedores_documentos_generales.findAll({
			where: {
				descripcion:{
		...
		const nuevoRegistro = await db.sequelize.models.proveedores_documentos_generales.create(registro);
...
async function show(req, res){
	...
	try {
		...
		const registroEncontrado = await db.sequelize.models.proveedores_documentos_generales.findByPk(id, {include:relaciones,paranoid: false});
...
async function update(req, res){
	...
	try {
		...
		const registroAEditar = await db.sequelize.models.proveedores_documentos_generales.findByPk(id);
		...
		const registrosEncontrados = await db.sequelize.models.proveedores_documentos_generales.findAll(whereFind);
		if(registrosEncontrados.length > 0){
			var regExistente = false
			await registrosEncontrados.forEach(registro => {
...
async function destroy(req,res){
	...
	try {
		const registroAEliminar = await db.sequelize.models.proveedores_documentos_generales.findByPk(id);
		if(registroAEliminar != null){
			...
			for (const modelo of Object.values(db.sequelize.models)) {
				...
				for (const asociacion of Object.values(asociaciones)) {
					if(asociacion.target.name == db.sequelize.models.proveedores_documentos_generales.name){
						let where = {}
...
async function restaurar(req,res){
	...
	try {
		const registroARestaurar = await db.sequelize.models.proveedores_documentos_generales.findByPk(id,{ paranoid: false });
