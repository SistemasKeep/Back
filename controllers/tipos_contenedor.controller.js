...
async function index(req, res) {
	...
	const camposModelo = Object.keys(db.sequelize.models.tipo_contenedor.rawAttributes);
	...
	try {
		const perfilesValidos = ['tamanios_contenedor', 'all']
		var relaciones = []
		...
		const docs = await db.sequelize.models.tipo_contenedor.findAll({
			paranoid: false,
			page: page || 1,
			include: relaciones,
		...
		const dataDocs = await db.sequelize.models.tipo_contenedor.count({
			paranoid: false,
			include: relaciones,
			where: filtro
		});
		...
		const fullUrl = `${req.protocol}://${req.get('host')}/api/tiposContenedor`;
...
async function store(req, res){
	...
	try {
		...
		const registrosEncontrados = await db.sequelize.models.tipo_contenedor.findAll({
			where: {
				descripcion: {
		...
		const nuevoRegistro = await db.sequelize.models.tipo_contenedor.create(registro);
...
async function show(req, res){
	...
	try {
		...
		const registroEncontrado = await db.sequelize.models.tipo_contenedor.findByPk(id, {include:relaciones,paranoid: false});
...
async function update(req, res){
	...
	try {
		...
		const registroAEditar = await db.sequelize.models.tipo_contenedor.findByPk(id);
		...
		const registrosEncontrados = await db.sequelize.models.tipo_contenedor.findAll(whereFind);
		if(registrosEncontrados.length > 0){
			var regExistente = false
			await registrosEncontrados.forEach(registro => {
...
async function destroy(req,res){
	...
	try {
		const registroAEliminar = await db.sequelize.models.tipo_contenedor.findByPk(id);
		if(registroAEliminar != null){
			...
			for (const modelo of Object.values(db.sequelize.models)) {
				...
				for (const asociacion of Object.values(asociaciones)) {
					if(asociacion.target.name == db.sequelize.models.tipo_contenedor.name){
						let where = {}
...
async function restaurar(req,res){
	...
	try {
		const registroARestaurar = await db.sequelize.models.tipo_contenedor.findByPk(id,{ paranoid: false });
