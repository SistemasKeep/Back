...
async function index(req, res) {
	...
	const camposModelo = Object.keys(db.sequelize.models.entidades_bancarias.rawAttributes);
	...
	try {
		...
		const docs = await db.sequelize.models.entidades_bancarias.findAll({
			paranoid: false,
			page: page || 1,
			include: relaciones,
		...
		const dataDocs = await db.sequelize.models.entidades_bancarias.count({
			paranoid: false,
			include: relaciones,
			where: filtro
		});
		...
		const fullUrl = `${req.protocol}://${req.get('host')}/api/entidadesBancarias`;
...
async function store(req, res){
	try {
		...
		const registrosEncontrados = await db.sequelize.models.entidades_bancarias.findAll({
			where: {
				nombre: {
		...
		const nuevoRegistro = await db.sequelize.models.entidades_bancarias.create(registro);
...
async function show(req, res){
	...
	try {
		...
		const registroEncontrado = await db.sequelize.models.entidades_bancarias.findByPk(id, {include:relaciones,paranoid: false});
...
async function update(req, res){
	...
	try {
		...
		const validosOpcionales = [{campo:'nombre', tipo:'string', largo:45, textoCase:"up"}];
		...
		const registroAEditar = await db.sequelize.models.entidades_bancarias.findByPk(id);
		...
		const registrosEncontrados = await db.sequelize.models.entidades_bancarias.findAll(whereFind);
		if(registrosEncontrados.length > 0){
			var regExistente = false
			await registrosEncontrados.forEach(registro => {
...
async function destroy(req,res){
	...
	try {
		const registroAEliminar = await db.sequelize.models.entidades_bancarias.findByPk(id);
		if(registroAEliminar != null){
			...
			for (const modelo of Object.values(db.sequelize.models)) {
				...
				for (const asociacion of Object.values(asociaciones)) {
					if(asociacion.target.name == db.sequelize.models.entidades_bancarias.name){
						let where = {}
...
async function restaurar(req,res){
	...
	try {
		const registroARestaurar = await db.sequelize.models.entidades_bancarias.findByPk(id,{ paranoid: false });
