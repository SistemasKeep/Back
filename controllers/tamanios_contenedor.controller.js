...
async function index(req, res) {
	...
	const camposModelo = Object.keys(db.sequelize.models.tamanios_contenedor.rawAttributes);
	...
	try {
		const perfilesValidos = ['tipo_contenedor', 'all']
		var relaciones = []
		...
		const docs = await db.sequelize.models.tamanios_contenedor.findAll({
			paranoid: false,
			page: page || 1,
			include: relaciones,
		...
		const dataDocs = await db.sequelize.models.tamanios_contenedor.count({
			paranoid: false,
			include: relaciones,
			where: filtro
		});
		...
		const fullUrl = `${req.protocol}://${req.get('host')}/api/tamaniosContenedor`;
...
async function store(req, res){
	...
	try {
		...
		let obligatorios = [{campo:'idTipoContenedor', tipo:'model',model:db.sequelize.models.tipo_contenedor},
		...
		const registrosEncontrados = await db.sequelize.models.tamanios_contenedor.findAll({
			where: {
				[db.Sequelize.Op.and]: {
		...
		const nuevoRegistro = await db.sequelize.models.tamanios_contenedor.create(registro);
...
async function show(req, res){
	...
	try {
		...
		const registroEncontrado = await db.sequelize.models.tamanios_contenedor.findByPk(id,{include:relaciones,paranoid: false});
...
async function update(req, res){
	...
	try {
		...
		const validosOpcionales = [{campo:'idTipoContenedor', tipo:'model',model:db.sequelize.models.tipo_contenedor},
								   {campo:'descripcion', tipo:'string',largo:255,textoCase:"up"}]
		...
		const registroAEditar = await db.sequelize.models.tamanios_contenedor.findByPk(id);
		...
		var whereFind = {
			where: {
				...
				deletedAt: null,
				descripcion: {[db.Sequelize.Op.like]: `%${parametros.descripcion != undefined ? parametros.descripcion : parametros.descripcion}%`}
			}
		}
		const registrosEncontrados = await db.sequelize.models.tamanios_contenedor.findAll(whereFind);
		if(registrosEncontrados.length > 0){
			var regExistente = false
			await registrosEncontrados.forEach(registro => {
...
async function destroy(req,res){
	...
	try {
		const registroAEliminar = await db.sequelize.models.tamanios_contenedor.findByPk(id);
		if(registroAEliminar != null){
			...
			for (const modelo of Object.values(db.sequelize.models)) {
				...
				for (const asociacion of Object.values(asociaciones)) {
					if(asociacion.target.name == db.sequelize.models.tamanios_contenedor.name){
						let where = {}
...
async function restaurar(req,res){
	...
	try {
		const registroARestaurar = await db.sequelize.models.tamanios_contenedor.findByPk(id,{ paranoid: false });
		if(registroARestaurar != null){
			if(registroARestaurar.deletedAt != null){
				const registrosEncontrados = await db.sequelize.models.tamanios_contenedor.findAll({
					where: {
						id_tipo_contenedor: registroARestaurar.id_tipo_contenedor,
						descripcion: {[db.Sequelize.Op.like]: `%${registroARestaurar.descripcion}%`},
						deletedAt: null
					}
				});
