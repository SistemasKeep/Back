...
async function index(req, res) {
	...
	const camposModelo = Object.keys(db.sequelize.models.polizas_nacionalidades_interes_asegurado.rawAttributes);
	...
	try {
		const perfilesValidos = ['poliza_detalle', 'pais', 'all']
		var relaciones = []
		if(perfilesValidos.includes(req.query.perfil)){
			const parametrosRelaciones = {
				poliza_detalle: [
					...
					'poliza_detalle.poliza.proveedor.estado.pais.continente',
			...
			try {
				const relacionesValidas = [ 
					...
					'poliza_detalle.poliza.proveedor.almacen.ubicacion_defecto',
					'poliza_detalle.poliza.proveedor.proveedor_tipo',
					'poliza_detalle.poliza.proveedor.estado',
					'poliza_detalle.poliza.proveedor.estado.pais',
					'poliza_detalle.poliza.proveedor.estado.pais.continente',
				]
		...
		const dataDocs = await db.sequelize.models.polizas_nacionalidades_interes_asegurado.count({
		    paranoid: false,
		    include: relaciones,
		    where: filtro
		});
		...
		const fullUrl = `${req.protocol}://${req.get('host')}/api/polizasNacionalidadesInteresAsegurado`;
...
async function store(req, res){
	try {
		...
		const registrosEncontrados = await db.sequelize.models.polizas_nacionalidades_interes_asegurado.findAll({
			where: {
				[db.Sequelize.Op.and]: {
		...
		const nuevoRegistro = await db.sequelize.models.polizas_nacionalidades_interes_asegurado.create(registro);
...
async function show(req, res){
	...
	try {
		...
		const registroEncontrado = await db.sequelize.models.polizas_nacionalidades_interes_asegurado.findByPk(id, {include:relaciones,paranoid: false});
...
async function update(req, res){
	...
	try {
		...
		const registroAEditar = await db.sequelize.models.polizas_nacionalidades_interes_asegurado.findByPk(id);
		...
		const registrosEncontrados = await db.sequelize.models.polizas_nacionalidades_interes_asegurado.findAll(whereFind);
		if(registrosEncontrados.length > 0){
			var regExistente = false
			await registrosEncontrados.forEach(registro => {
				if((registro.id_poliza_detalle == parametros.idPolizaDetalle != undefined ? parametros.idPolizaDetalle : registroAEditar.id_poliza_detalle &&
					registro.id_pais == parametros.idPais != undefined ? parametros.idPais : registroAEditar.id_pais) &&
				    registro.id != id){
						regExistente = true;
						res.status(400).send({ status: false, msg: "Registro existente"});
				}
		...
		var registro2 = {
			...
			tabla: db.sequelize.models.polizas_nacionalidades_interes_asegurado.name.toUpperCase(),
			accion: 'EDICION',
			createdAt: moment().tz('America/Mexico_City')
		}
...
async function destroy(req,res){
	...
	try {
		const registroAEliminar = await db.sequelize.models.polizas_nacionalidades_interes_asegurado.findByPk(id);
		if(registroAEliminar != null){
			...
			for (const modelo of Object.values(db.sequelize.models)) {
				...
				for (const asociacion of Object.values(asociaciones)) {
					if(asociacion.target.name == db.sequelize.models.polizas_nacionalidades_interes_asegurado.name){
						let where = {}
...
async function restaurar(req,res){
	...
	try {
		const registroARestaurar = await db.sequelize.models.polizas_nacionalidades_interes_asegurado.findByPk(id,{ paranoid: false });
...
async function showHistoricos(req, res) {
	...
	if(registro.tabla != db.sequelize.models.polizas_nacionalidades_interes_asegurado.name.toUpperCase()){
		return res.status(400).send({ status: false, msg: "Registro no pertenece al crud polizas_nacionalidades_interes_asegurado" });
	} 
	let usuario = await db.sequelize.models.usuarios.findByPk(registro.id_usuario_registro);
	reg.id = registro.id;
	reg.usuario_registro = {id: usuario.id, nombre: usuario.nombre};
