...
async function index(req, res) {
	...
	try {
		...
		const docs = await db.sequelize.models.buques.findAll({
			paranoid: false,
			attributes: req.query.keepro !== 3 ? undefined : ['id', 'id_tipo_buque', 'nombre'],
			page: page || 1,
			include: relaciones,
		...
		const dataDocs = await db.sequelize.models.buques.count({
			paranoid: false,
			include: relaciones,
			where: filtro
		});
		...
		const fullUrl = `${req.protocol}://${req.get('host')}/api${req.path}`;
		...
		for(const doc of docs){
			...
			if(req.query.keepro === 3 ){
				element.id_tipo_buque = undefined
				element.tipo_buque.id = undefined
				element.tipo_buque.id_usuario_registro = undefined
				element.tipo_buque.createdAt = undefined
				element.tipo_buque.updatedAt = undefined
				element.tipo_buque.deletedAt = undefined
...
async function store(req, res){
	...
	try {
		...
		let obligatorios = [{campo:'idTipoBuque', tipo:'model',model:db.sequelize.models.tipos_buque},
							{campo:'nombre', tipo:'string',largo:255,textoCase:"up"},
							{campo:'companiaTransporte', tipo:'string',largo:100,textoCase:"up"},
							{campo:'numeroOmi', tipo:'stringInt',largo:20},
							{campo:'anioConstruccion', tipo:'number'}]
		registro = await Validaciones.validParametros(req, res,obligatorios,registro);
		if(!registro){
			return '';
		}
		const tipoBuqueEncontrado = await db.sequelize.models.tipos_buque.findByPk(parametros.idTipoBuque);
		if(tipoBuqueEncontrado == undefined){
			return res.status(400).send({ status: false, msg: "Tipo buque no válido" });
		}
		if(tipoBuqueEncontrado.deletedAt != null){
			return res.status(400).send({ status: false, msg: "Tipo buque eliminado" });
		}
	} catch (error) {
		return res.status(500).send({ status: false, msg: "Error interno del servidor", error: error.toString()});
	} 
	try {
		const registrosEncontrados = await db.sequelize.models.buques.findAll({
			where: {
				[db.Sequelize.Op.and]: {
					deletedAt: null
				},
				[db.Sequelize.Op.or]: {
					...
					numero_omi: {
						[db.Sequelize.Op.like]: `%${parametros.numeroOmi}%`
					}
				}
			}
		});
		if(registrosEncontrados.length > 0){
			...
			await registrosEncontrados.forEach(registro => {
				if(registro.nombre.toLowerCase() == parametros.nombre.toLowerCase() || 
				   registro.numero_omi == parametros.numeroOmi){
					if(!regExistente){
						regExistente = true;
		...
		const nuevoRegistro = await db.sequelize.models.buques.create(registro);
...
async function show(req, res){
	...
	try {
		...
		const registroEncontrado = await db.sequelize.models.buques.findByPk(id,{include:relaciones,paranoid: false});
...
async function update(req, res){
	...
	try {
		...
		const validosOpcionales =[{campo:'idTipoBuque',tipo:'model',model:db.sequelize.models.tipos_buque},
								  {campo:'nombre',tipo:'string',textoCase:"up",largo:255},
								  {campo:'companiaTransporte',tipo:'string',textoCase:"up",largo:100},
								  {campo:'numeroOmi',tipo:'stringInt',largo:20},
								  {campo:'anioConstruccion',tipo:'number'},
		...
		const registroAEditar = await db.sequelize.models.buques.findByPk(id);
		...
		var whereFind = {
			where: {
				...
				[db.Sequelize.Op.or]: {
					numero_omi: {
						[db.Sequelize.Op.like]: `%${parametros.numeroOmi != undefined ? parametros.numeroOmi : registroAEditar.numero_omi}%`
					},
					nombre: {
		...
		const registrosEncontrados = await db.sequelize.models.buques.findAll(whereFind);
		if(registrosEncontrados.length > 0){
			var regExistente = false
			await registrosEncontrados.forEach(registro => {
				if((registro.nombre.toLowerCase() == (parametros.nombre != undefined ? parametros.nombre.toLowerCase() : registroAEditar.nombre.toLowerCase()) || 
				   registro.numero_omi == (parametros.numeroOmi != undefined ? parametros.numeroOmi : registroAEditar.numero_omi)) &&
				   registro.id != id){
					if(!regExistente){
						regExistente = true;
						res.status(400).send({ status: false, msg: "Registro existente"});
		...
		var registro2 = {
			...
			tabla: db.sequelize.models.buques.name.toUpperCase(),
			accion: 'EDICION',
			createdAt: moment().tz('America/Mexico_City')
		}
...
async function destroy(req,res){
	...
	try {
		const registroAEliminar = await db.sequelize.models.buques.findByPk(id);
		if(registroAEliminar != null){
			...
			for (const modelo of Object.values(db.sequelize.models)) {
				...
				for (const asociacion of Object.values(asociaciones)) {
					if(asociacion.target.name == db.sequelize.models.buques.name){
						let where = {}
...
async function restaurar(req,res){
	...
	try {
		const registroARestaurar = await db.sequelize.models.buques.findByPk(id,{ paranoid: false });
				...
				const registrosEncontrados = await db.sequelize.models.buques.findAll({
					where: {
						...
						[db.Sequelize.Op.or]:{
							...
							numero_omi: {
								[db.Sequelize.Op.like]: `%${registroARestaurar.numeroOmi}%`
							}
						}
					}
				});
				...
				if(registrosEncontrados.length > 0){
					...
					await registrosEncontrados.forEach(registro => {
						if((registro.nombre.toLowerCase() == registroARestaurar.nombre.toLowerCase() || 
						   registro.numero_omi == registroARestaurar.numeroOmi) &&
						   registro.id != id){
							if(!regExistente){
								regExistente = true;
								res.status(400).send({ status: false, msg: "Registro existente"});
...
async function showHistoricos(req, res) {
	...
	if(registro.tabla != db.sequelize.models.buques.name.toUpperCase()){
		return res.status(400).send({ status: false, msg: "Registro no pertenece al crud buques" });
	} 
	let usuario = await db.sequelize.models.usuarios.findByPk(registro.id_usuario_registro);
	reg.id = registro.id;
	reg.usuario_registro = {id: usuario.id, nombre: usuario.nombre};
...
async function exportacion(req, res) {
    ...
    try {
        ...
        for(const element of docs){
            ...
            elementos.push({
                'Tipo': element.tipo_buque.descripcion,
                'Nombre': element.nombre,
                'Compañia de Transporte': element.compania_transporte,
                'Número Omi': element.numero_omi,
				'Año de Construcción': element.anio_construccion,
            });
        }
        ...
        const nombreReporte = `buques_${moment().tz('America/Mexico_City').format('YYYY-MM-DD')}`;
        const namesSheets = [db.sequelize.models.buques.name];
        const reporteBuques = new ReportesXLSX({
            nombreReporte:nombreReporte,
            elementos:elementos,
            namesSheets:namesSheets, 
            idMarca:idMarca
        });
        return await reporteBuques.gerReporteOneSheet(res,req);
    } catch (error) {
