...
async function index(req, res) {
	...
	const camposModelo = Object.keys(db.sequelize.models.fuentes.rawAttributes);
	...
	try {
		...
		const docs = await db.sequelize.models.fuentes.findAll({
			paranoid: false,
			page: page || 1,
			include:relaciones, 
		...
		const dataDocs = await db.sequelize.models.fuentes.count({
			paranoid: false,
			include:relaciones, 
			where: filtro
		});
		...
		const fullUrl = `${req.protocol}://${req.get('host')}/api/fuentes`;
...
async function store(req, res){
	try {
		...
		let obligatorios = [{campo:'descripcion', tipo:'string', largo:100, textoCase:"up"}
        ]
		...
		const registrosEncontrados = await db.sequelize.models.fuentes.findAll({
			where: {
				descripcion:{
		...
		const nuevoRegistro = await db.sequelize.models.fuentes.create(registro);
...
async function show(req, res){
	...
	try {
		const findRelaciones = new Relaciones([],[],db.sequelize.models)
		const relaciones = await findRelaciones.getRelaciones()
		const registroEncontrado = await db.sequelize.models.fuentes.findByPk(id, {include:relaciones, paranoid: false});
...
async function update(req, res){
	...
	try {
		...
		const validosOpcionales = [{campo:'descripcion', tipo:'string', textoCase:"up", largo:100}
        ]
		...
		const registroAEditar = await db.sequelize.models.fuentes.findByPk(id);
		...
		const registrosEncontrados = await db.sequelize.models.fuentes.findAll(whereFind);
		if(registrosEncontrados.length > 0){
			var regExistente = false
			await registrosEncontrados.forEach(registro => {
...
async function destroy(req,res){
	...
	try {
		const registroAEliminar = await db.sequelize.models.fuentes.findByPk(id);
		if(registroAEliminar != null){
			...
			for (const modelo of Object.values(db.sequelize.models)) {
				...
				for (const asociacion of Object.values(asociaciones)) {
					if(asociacion.target.name == db.sequelize.models.fuentes.name){
						let where = {}
...
async function restaurar(req,res){
	...
	try {
		const registroARestaurar = await db.sequelize.models.fuentes.findByPk(id,{ paranoid: false });
