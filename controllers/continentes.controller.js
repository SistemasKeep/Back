...
async function index(req, res) {
	...
	const camposModelo = Object.keys(db.sequelize.models.continentes.rawAttributes);
	...
	try {
		...
		const docs = await db.sequelize.models.continentes.findAll({
			paranoid: false,
			page: page || 1,
			include: relaciones,
		...
		const dataDocs = await db.sequelize.models.continentes.count({
			paranoid: false,
			include: relaciones,
			where: filtro
		});
		...
		const fullUrl = `${req.protocol}://${req.get('host')}/api/continentes`;
		...
		for(const doc of docs){
			...
			/*if(req.query.perfil == 'all'){
				element.paises = await db.sequelize.models.paises.findAll({where:{id_continente: element.id}})
			}*/
			data.push(element)
		}
...
async function store(req, res){
	...
	try {
		...
		const registrosEncontrados = await db.sequelize.models.continentes.findAll({
			where: {
				nombre: {
		...
		const nuevoRegistro = await db.sequelize.models.continentes.create(registro);
...
async function show(req, res){
	...
	try {
		...
		const findRelaciones = new Relaciones([],[],db.sequelize.models)
		const relaciones = await findRelaciones.getRelaciones()
		const registroEncontrado = await db.sequelize.models.continentes.findByPk(id,{include:relaciones,paranoid: false});
...
async function update(req, res){
	...
	try {
		...
		const validosOpcionales =[{campo:'nombre',tipo:'string',textoCase:"up",largo:255}]
		...
		const registroAEditar = await db.sequelize.models.continentes.findByPk(id);
		...
		const registrosEncontrados = await db.sequelize.models.continentes.findAll(whereFind);
		if(registrosEncontrados.length > 0){
			var regExistente = false
			await registrosEncontrados.forEach(registro => {
		...
		var registro2 = {
			...
			tabla: db.sequelize.models.continentes.name.toUpperCase() ,
			accion: 'EDICION',
			createdAt: moment().tz('America/Mexico_City')
		}
...
async function destroy(req,res){
	...
	try {
		const registroAEliminar = await db.sequelize.models.continentes.findByPk(id);
		if(registroAEliminar != null){
			...
			for (const modelo of Object.values(db.sequelize.models)) {
				...
				for (const asociacion of Object.values(asociaciones)) {
					if(asociacion.target.name == db.sequelize.models.continentes.name){
						let where = {}
...
async function restaurar(req,res){
	...
	try {
		const registroARestaurar = await db.sequelize.models.continentes.findByPk(id,{ paranoid: false });
		if(registroARestaurar != null){
			if(registroARestaurar.deletedAt != null){
				...
				if(registrosEncontrados.length > 0){
					var regExistente = false
					await registrosEncontrados.forEach(registro => {
						if(registro.nombre.toLowerCase() == registroARestaurar.nombre.toLowerCase() && registro.id != id){
							if(!regExistente){
								regExistente = true;
								res.status(400).send({ status: false, msg: "Registro existente"});
							}
...
async function showHistoricos(req, res) {
	...
	if(registro.tabla != db.sequelize.models.continentes.name.toUpperCase()){
		return res.status(400).send({ status: false, msg: "Registro no pertenece al crud continentes" });
	} 
	let usuario = await db.sequelize.models.usuarios.findByPk(registro.id_usuario_registro);
	reg.id = registro.id;
	reg.usuario_registro = {id: usuario.id, nombre: usuario.nombre};
