...
async function index(req, res) {
	...
	const camposModelo = Object.keys(db.sequelize.models.proveedores_expediente.rawAttributes);
	...
	try {
		const perfilesValidos = ['proveedor', 'documento_proveedor', 'archivo', 'all']
		var relaciones = []
		if(perfilesValidos.includes(req.query.perfil)){
			const parametrosRelaciones = {
				...
				archivo: ['archivo'],
				all: [ 'documento_proveedor', 'proveedor.moneda', 'proveedor.conceptos_presupuesto', 'proveedor.marca.domicilio.estado.pais.continente', 'proveedor.marca.pais.continente', 'proveedor.marca.archivo', 'proveedor.marca.dato_facturacion.regimen_fiscal',  'proveedor.marca.dato_facturacion.pais.continente',  'proveedor.marca.dato_facturacion.nacionalidad_timbrado.continente', 'proveedor.almacen.marca.domicilio.estado.pais.continente', 'proveedor.almacen.marca.pais.continente', 'proveedor.almacen.marca.archivo', 'proveedor.almacen.marca.dato_facturacion.regimen_fiscal',  'proveedor.almacen.marca.dato_facturacion.pais.continente',  'proveedor.almacen.marca.dato_facturacion.nacionalidad_timbrado.continente', 'proveedor.almacen.ubicacion_defecto', 'proveedor.proveedor_tipo',  'archivo' ]
			}
		...
		const docs = await db.sequelize.models.proveedores_expediente.findAll({
			paranoid: false,
			page: page || 1,
			include: relaciones,
		...
		const dataDocs = await db.sequelize.models.proveedores_expediente.count({
		    paranoid: false,
		    include: relaciones,
		    where: filtro
		});
		...
		const fullUrl = `${req.protocol}://${req.get('host')}/api/proveedoresExpediente`;
...
async function store(req, res){
	try {
		...
		const registrosEncontrados = await db.sequelize.models.proveedores_expediente.findAll({
			where: {
		...
		if(registrosEncontrados.length > 0){
			...
			await registrosEncontrados.forEach(registro => {
				if(registro.descripcion.toLowerCase() == parametros.descripcion.toLowerCase() &&
				   registro.id_proveedor == parametros.idProveedor){
					if(!regExistente){
						regExistente = true;
		...
		const nuevoRegistro = await db.sequelize.models.proveedores_expediente.create(registro);
...
async function show(req, res){
	...
	try {
		...
		const registroEncontrado = await db.sequelize.models.proveedores_expediente.findByPk(id, {include:relaciones,paranoid: false});
...
async function update(req, res){
	...
	try {
		...
		const registroAEditar = await db.sequelize.models.proveedores_expediente.findByPk(id);
		...
		const registrosEncontrados = await db.sequelize.models.proveedores_expediente.findAll(whereFind);
		if(registrosEncontrados.length > 0){
			var regExistente = false
			await registrosEncontrados.forEach(registro => {
...
async function destroy(req,res){
	...
	try {
		const registroAEliminar = await db.sequelize.models.proveedores_expediente.findByPk(id);
		if(registroAEliminar != null){
			...
			for (const modelo of Object.values(db.sequelize.models)) {
				...
				for (const asociacion of Object.values(asociaciones)) {
					if(asociacion.target.name == db.sequelize.models.proveedores_expediente.name){
						let where = {}
...
async function restaurar(req,res){
	...
	try {
		const registroARestaurar = await db.sequelize.models.proveedores_expediente.findByPk(id,{ paranoid: false });
				...
				if(registrosEncontrados.length > 0){
					...
					await registrosEncontrados.forEach(registro => {
						if(registro.descripcion.toLowerCase() == registroARestaurar.descripcion.toLowerCase() &&
						   registro.id_proveedor == registroARestaurar.id_proveedor &&
						   registro.id != id){
							if(!regExistente){
								regExistente = true;
								res.status(400).send({ status: false, msg: "Registro existente"});
