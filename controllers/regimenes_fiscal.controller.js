...
async function index(req, res) {
	...
	const camposModelo = Object.keys(db.sequelize.models.regimenes_fiscal.rawAttributes);
	...
	try {
		...
		const docs = await db.sequelize.models.regimenes_fiscal.findAll({
			paranoid: false,
			include: relaciones,
			page: page || 1,
		...
		const dataDocs = await db.sequelize.models.regimenes_fiscal.count({
			paranoid: false,
			include: relaciones,
			where: filtro
		});
		...
		const fullUrl = `${req.protocol}://${req.get('host')}/api/regimenesFiscal`;
...
async function store(req, res){
	...
	try {
		...
		let obligatorios = [{campo:'descripcion', tipo:'string',largo:255,textoCase:"up"},
							...
							{campo:'tipoPersona', tipo:'enum', largo:2, textoCase:"up", enum: ['F', 'M', 'FM']}]
		registro = await Validaciones.validParametros(req, res,obligatorios,registro);
		if(!registro){
			return '';
		}
		const registrosEncontrados = await db.sequelize.models.regimenes_fiscal.findAll({
			where: {
				[db.Sequelize.Op.and]: {
					deletedAt: null
				},
		...
		const nuevoRegistro = await db.sequelize.models.regimenes_fiscal.create(registro);
...
async function show(req, res){
	...
	try {
		const findRelaciones = new Relaciones([],[],db.sequelize.models)
		const relaciones = await findRelaciones.getRelaciones()
		const registroEncontrado = await db.sequelize.models.regimenes_fiscal.findByPk(id,{ paranoid: false,include: relaciones, });
...
async function update(req, res){
	...
	try {
		...
		const registroAEditar = await db.sequelize.models.regimenes_fiscal.findByPk(id);
		...
		const registrosEncontrados = await db.sequelize.models.regimenes_fiscal.findAll(whereFind);
		if(registrosEncontrados.length > 0){
			var regExistente = false
			await registrosEncontrados.forEach(registro => {
...
async function destroy(req,res){
	...
	try {
		const registroAEliminar = await db.sequelize.models.regimenes_fiscal.findByPk(id);
		if(registroAEliminar != null){
			...
			for (const modelo of Object.values(db.sequelize.models)) {
				...
				for (const asociacion of Object.values(asociaciones)) {
					if(asociacion.target.name == db.sequelize.models.regimenes_fiscal.name){
						let where = {}
...
async function restaurar(req,res){
	...
	try {
		const registroARestaurar = await db.sequelize.models.regimenes_fiscal.findByPk(id,{ paranoid: false });
