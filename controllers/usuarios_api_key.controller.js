...
async function index(req, res) {
	...
	const camposModelo = Object.keys(db.sequelize.models.usuarios_api_key.rawAttributes);
	...
	try {
		const perfilesValidos = ['usuario'];
		var relaciones = [];
		if(perfilesValidos.includes(req.query.perfil)){
			const parametrosRelaciones = {usuario: ['usuario']};
		...
		const docs = await db.sequelize.models.usuarios_api_key.findAll({
			paranoid: false,
			page: page || 1,
			include: relaciones,
		...
		const totalCount = await db.sequelize.models.usuarios_api_key.count({
			paranoid: false,
			include: relaciones,
			where: filtro
		});
		...
		const fullUrl = `${req.protocol}://${req.get('host')}/api/usuariosApiKey`;
...
async function store(req, res){
	try {
		...
		const registrosEncontrados = await db.sequelize.models.usuarios_api_key.findAll({
			where: {
				[db.Sequelize.Op.and]: {
		...
		if(registrosEncontrados.length > 0){
			...
			await registrosEncontrados.forEach(registro => {
				if(registro.id_usuario == parametros.idUsuario && registro.testeo == parametros.testeo){
					if(!regExistente){
						regExistente = true;
		...
		const nuevoRegistro = await db.sequelize.models.usuarios_api_key.create(registro);
...
async function show(req, res){
	...
	try {
		...
		const registroEncontrado = await db.sequelize.models.usuarios_api_key.findByPk(id, {include:relaciones,paranoid: false});
...
async function update(req, res){
	...
	try {
		...
		const registroAEditar = await db.sequelize.models.usuarios_api_key.findByPk(id);
        ...
        const id_usuario = parametros.idUsuario != undefined ? parametros.idUsuario :registroAEditar.id_usuario;
		...
		const registrosEncontrados = await db.sequelize.models.usuarios_api_key.findAll(whereFind);
		if(registrosEncontrados.length > 0){
			var regExistente = false
			await registrosEncontrados.forEach(registro => {
				if(registro.id_usuario == id_usuario && registro.testeo == testeo && registro.id != id){
					if(!regExistente){
						regExistente = true;
...
async function destroy(req,res){
	...
	try {
		const registroAEliminar = await db.sequelize.models.usuarios_api_key.findByPk(id);
		if(registroAEliminar != null){
			...
			for (const modelo of Object.values(db.sequelize.models)) {
				...
				for (const asociacion of Object.values(asociaciones)) {
					if(asociacion.target.name == db.sequelize.models.usuarios_api_key.name){
						let where = {}
...
async function restaurar(req,res){
	...
	try {
		const registroARestaurar = await db.sequelize.models.usuarios_api_key.findByPk(id,{ paranoid: false });
		if(registroARestaurar != null){
			if(registroARestaurar.deletedAt != null){
				...
				if(registrosEncontrados.length > 0){
					var regExistente = false
					await registrosEncontrados.forEach(registro => {
						if(registro.id_usuario == registroARestaurar.id_usuario && registro.key == registroARestaurar.key && registro.id != id){
							if(!regExistente){
								regExistente = true;
								res.status(400).send({ status: false, msg: "Registro existente"});
							}
