...
async function index(req, res) {
	...
	const camposModelo = Object.keys(db.sequelize.models.marcas_monedas.rawAttributes);
	...
	try {
		const perfilesValidos = ['marca', 'moneda', 'all']
		var relaciones = []
		if(perfilesValidos.includes(req.query.perfil)){
			const parametrosRelaciones = {
				...
				moneda: [ 'moneda' ],
				all: [ 'moneda','marca.domicilio.estado.pais.continente','marca.pais.continente','marca.archivo','marca.dato_facturacion.regimen_fiscal', 'marca.dato_facturacion.pais.continente', 'marca.dato_facturacion.nacionalidad_timbrado.continente' ]
			}
		...
		const docs = await db.sequelize.models.marcas_monedas.findAll({
			paranoid: false,
			page: page || 1,
			include: relaciones,
		...
		const dataDocs = await db.sequelize.models.marcas_monedas.count({
			paranoid: false,
			include: relaciones,
			where: filtro
		});
		...
		const fullUrl = `${req.protocol}://${req.get('host')}/api/marcasMonedas`;
...
async function store(req, res){
	...
	try {
		...
		const registrosEncontrados = await db.sequelize.models.marcas_monedas.findAll({
			where: {
		...
		if(registrosEncontrados.length > 0){
			...
			await registrosEncontrados.forEach(registro => {
				if(registro.id_marca == parametros.idMarca &&
					registro.id_moneda ==  parametros.idMoneda){
						if(!regExistente){
							regExistente = true;
		...
		const nuevoRegistro = await db.sequelize.models.marcas_monedas.create(registro);
...
async function show(req, res){
	...
	try {
		...
		const registroEncontrado = await db.sequelize.models.marcas_monedas.findByPk(id,{include:relaciones,paranoid: false});
...
async function update(req, res){
	...
	try {
		...
		const registroAEditar = await db.sequelize.models.marcas_monedas.findByPk(id);
		...
		const registrosEncontrados = await db.sequelize.models.marcas_monedas.findAll(whereFind);
		if(registrosEncontrados.length > 0){
			var regExistente = false
			await registrosEncontrados.forEach(registro => {
				if(((registro.id_marca == (parametros.idMarca != undefined ? parametros.idMarca : registroAEditar.id_marca)) &&
				    (registro.id_moneda == (parametros.idMoneda != undefined ? parametros.idMoneda : registroAEditar.id_moneda))) &&
				    registro.id != id){
						if(!regExistente){
							regExistente = true;
							res.status(400).send({ status: false, msg: "Registro existente"});
...
async function destroy(req,res){
	...
	try {
		const registroAEliminar = await db.sequelize.models.marcas_monedas.findByPk(id);
		if(registroAEliminar != null){
			...
			for (const modelo of Object.values(db.sequelize.models)) {
				...
				for (const asociacion of Object.values(asociaciones)) {
					if(asociacion.target.name == db.sequelize.models.marcas_monedas.name){
						let where = {}
...
async function restaurar(req,res){
	...
	try {
		const registroARestaurar = await db.sequelize.models.marcas_monedas.findByPk(id,{ paranoid: false });
				...
				if(registrosEncontrados.length > 0){
					...
					await registrosEncontrados.forEach(registro => {
						if((registro.id_marca == registroARestaurar.id_marca &&
							registro.id_moneda == registroARestaurar.id_moneda) &&
							registro.id != id){
								if(!regExistente){
									regExistente = true;
									res.status(400).send({ status: false, msg: "Registro existente"});
