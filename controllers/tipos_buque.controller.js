...
async function index(req, res) {
	...
	const camposModelo = Object.keys(db.sequelize.models.tipos_buque.rawAttributes);
	...
	try {
		...
		const docs = await db.sequelize.models.tipos_buque.findAll({
			paranoid: false,
			page: page || 1,
			include: relaciones,
		...
		const dataDocs = await db.sequelize.models.tipos_buque.count({
			paranoid: false,
			include: relaciones,
			where: filtro
		});
		...
		const fullUrl = `${req.protocol}://${req.get('host')}/api/tiposBuque`;
...
async function store(req, res){
	...
	try {
		...
		var totalCount = await db.sequelize.models.tipos_buque.count();
		totalCount = totalCount + 1
		parametros.clave = "b-" + totalCount
		...
		const registrosEncontrados = await db.sequelize.models.tipos_buque.findAll({
			where: {
				[db.Sequelize.Op.and]: {
					deletedAt: null
				},
		...
		const nuevoRegistro = await db.sequelize.models.tipos_buque.create(registro);
...
async function show(req, res){
	...
	try {
		const findRelaciones = new Relaciones([],[],db.sequelize.models)
		const relaciones = await findRelaciones.getRelaciones()
		const registroEncontrado = await db.sequelize.models.tipos_buque.findByPk(id,{ paranoid: false,include: relaciones });
...
async function update(req, res){
	...
	try {
		...
		const registroAEditar = await db.sequelize.models.tipos_buque.findByPk(id);
		...
		const registrosEncontrados = await db.sequelize.models.tipos_buque.findAll(whereFind);
		if(registrosEncontrados.length > 0){
			var regExistente = false
			await registrosEncontrados.forEach(registro => {
		...
		var registro2 = {
			...
			tabla: db.sequelize.models.tipos_buque.name.toUpperCase() ,
			accion: 'EDICION',
			createdAt: moment().tz('America/Mexico_City')
		}
...
async function destroy(req,res){
	...
	try {
		const registroAEliminar = await db.sequelize.models.tipos_buque.findByPk(id);
		if(registroAEliminar != null){
			...
			for (const modelo of Object.values(db.sequelize.models)) {
				...
				for (const asociacion of Object.values(asociaciones)) {
					if(asociacion.target.name == db.sequelize.models.tipos_buque.name){
						let where = {}
...
async function restaurar(req,res){
	...
	try {
		const registroARestaurar = await db.sequelize.models.tipos_buque.findByPk(id,{ paranoid: false });
...
async function showHistoricos(req, res) {
	...
	if(registro.tabla != db.sequelize.models.tipos_buque.name.toUpperCase()){
		return res.status(400).send({ status: false, msg: "Registro no pertenece al crud tipos_buque" });
	} 
	let usuario = await db.sequelize.models.usuarios.findByPk(registro.id_usuario_registro);
	reg.id = registro.id;
	reg.usuario_registro = {id: usuario.id, nombre: usuario.nombre};
...
async function exportacion(req, res) {
    ...
    try {
        ...
        const nombreReporte = `tipos_buque_${moment().tz('America/Mexico_City').format('YYYY-MM-DD')}`;
        const namesSheets = [db.sequelize.models.tipos_buque.name];
        const reporteUnidadesMedida = new ReportesXLSX({
            nombreReporte:nombreReporte,
            elementos:elementos,
            namesSheets:namesSheets, 
            idMarca:idMarca
        });
        return await reporteUnidadesMedida.gerReporteOneSheet(res,req);
    } catch (error) {
        return res.status(500).json({ success: false, error: 'Error interno del servidor', error: error.toString() });
    }
