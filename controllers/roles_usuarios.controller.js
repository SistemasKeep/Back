...
async function index(req, res) {
	...
	const camposModelo = Object.keys(db.sequelize.models.roles_usuarios.rawAttributes);
	...
	try {
		const perfilesValidos = ['usuario', 'rol', 'all']
		var relaciones = []
		...
		const docs = await db.sequelize.models.roles_usuarios.findAll({
			paranoid: false,
			page: page || 1,
			include: relaciones,
		...
		const dataDocs = await db.sequelize.models.roles_usuarios.count({
			paranoid: false,
			include: relaciones,
			where: filtro
		});
		...
		const fullUrl = `${req.protocol}://${req.get('host')}/api/rolesUsuarios`;
...
async function store(req, res){
	try {
		...
		const registrosEncontrados = await db.sequelize.models.roles_usuarios.findAll({
			where: {
				[db.Sequelize.Op.and]: {
		...
		if(registrosEncontrados.length > 0){
			...
			await registrosEncontrados.forEach(registro => {
				if(registro.id_role == parametros.idRole || registro.id_usuario == parametros.idUsuario){
					regExistente = true;
		...
		const nuevoRegistro = await db.sequelize.models.roles_usuarios.create(registro);
...
async function show(req, res){
	...
	try {
		...
		const registroEncontrado = await db.sequelize.models.roles_usuarios.findByPk(id,{include:relaciones,paranoid: false});
...
async function update(req, res){
	...
	try {
		...
		const registroAEditar = await db.sequelize.models.roles_usuarios.findByPk(id);
		...
		const registrosEncontrados = await db.sequelize.models.roles_usuarios.findAll(whereFind);
		if(registrosEncontrados.length > 0){
			var regExistente = false
			await registrosEncontrados.forEach(registro => {
...
async function destroy(req,res){
	...
	try {
		const registroAEliminar = await db.sequelize.models.roles_usuarios.findByPk(id);
		if(registroAEliminar != null){
			...
			for (const modelo of Object.values(db.sequelize.models)) {
				...
				for (const asociacion of Object.values(asociaciones)) {
					if(asociacion.target.name == db.sequelize.models.roles_usuarios.name){
						let where = {}
...
async function restaurar(req,res){
	...
	try {
		const registroARestaurar = await db.sequelize.models.roles_usuarios.findByPk(id,{ paranoid: false });
		if(registroARestaurar != null){
			if(registroARestaurar.deletedAt != null){
				...
				if(registrosEncontrados.length > 0){
					var regExistente = false
					await registrosEncontrados.forEach(registro => {
						if((registro.id_role == registroARestaurar.id_role && registro.id_usuario == registroARestaurar.id_usuario) &&
							registro.id != id){
								regExistente = true;
