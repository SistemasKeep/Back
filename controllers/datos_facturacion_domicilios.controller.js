...
async function index(req, res) {
	...
	const camposModelo = Object.keys(db.sequelize.models.datos_facturacion_domicilios.rawAttributes);
	...
	try {
		const perfilesValidos = ['dato_facturacion', 'domicilio', 'all']
		var relaciones = []
		if(perfilesValidos.includes(req.query.perfil)){
			const parametrosRelaciones = {
				...
				all: [
					'dato_facturacion.regimen_fiscal', 'dato_facturacion.pais.continente', 'dato_facturacion.nacionalidad_timbrado.continente','domicilio.estado.pais.continente'
				]
			}
		...
		const docs = await db.sequelize.models.datos_facturacion_domicilios.findAll({
			paranoid: false,
			page: page || 1,
			include: relaciones,
		...
		const dataDocs = await db.sequelize.models.datos_facturacion_domicilios.count({
			paranoid: false,
			include: relaciones,
			where: filtro
		});
		...
		const fullUrl = `${req.protocol}://${req.get('host')}/api/datosFacturacionDomicilios`;
...
async function store(req, res){
	...
	try {
		...
		let obligatorios = [{campo:'idDatoFacturacion',  tipo:'model', model:db.sequelize.models.datos_facturacion},
		...
		const registrosEncontrados = await db.sequelize.models.datos_facturacion_domicilios.findAll({
			where: {
				deletedAt: null,
				[db.Sequelize.Op.or]: [
					...
					{
						[db.Sequelize.Op.and]: [
							{ id_dato_facturacion: parametros.idDatoFacturacion },
							{ tipo: 'F' }
						]
					}
				]
		...
		if(registrosEncontrados.length > 0){
			...
			await registrosEncontrados.forEach(registro => {
				if((registro.id_domicilio == parametros.idDomicilio) || 
				   (registro.id_dato_facturacion == parametros.idDatoFacturacion && (parametros.tipo == "F" && registro.tipo == "F"))){
					if(!regExistente){
						regExistente = true;
		...
		const nuevoRegistro = await db.sequelize.models.datos_facturacion_domicilios.create(registro);
...
async function show(req, res){
	...
	try {
		...
		const registroEncontrado = await db.sequelize.models.datos_facturacion_domicilios.findByPk(id, {include:relaciones,paranoid: false});
...
async function update(req, res){
	...
	try {
		...
		const registroAEditar = await db.sequelize.models.datos_facturacion_domicilios.findByPk(id);
		...
		var whereFind = {
			where: {
				...
				[db.Sequelize.Op.or]: [
					...
					{
						[db.Sequelize.Op.and]: [
							{ id_dato_facturacion: parametros.idDatoFacturacion != undefined ? parametros.idDatoFacturacion : registroAEditar.id_dato_facturacion },
							{ tipo: 'F' }
						]
					}
				]
		...
		const registrosEncontrados = await db.sequelize.models.datos_facturacion_domicilios.findAll(whereFind);
		if(registrosEncontrados.length > 0){
			var regExistente = false
			await registrosEncontrados.forEach(registro => {
				if((registro.id_domicilio == (parametros.idDomicilio != undefined ? parametros.idDomicilio : registroAEditar.id_domicilio)) || 
				   (registro.id_dato_facturacion == (parametros.idDatoFacturacion != undefined ? parametros.idDatoFacturacion : registroAEditar.id_dato_facturacion) && (((parametros.tipo != undefined ? parametros.tipo : registroAEditar.tipo) == "F") && registro.tipo == "F")) &&
				   registro.id != id){
					if(!regExistente){
						regExistente = true;
						res.status(400).send({ status: false, msg: "Registro existente"});
...
async function destroy(req,res){
	...
	try {
		const registroAEliminar = await db.sequelize.models.datos_facturacion_domicilios.findByPk(id);
		if(registroAEliminar != null){
			...
			for (const modelo of Object.values(db.sequelize.models)) {
				...
				for (const asociacion of Object.values(asociaciones)) {
					if(asociacion.target.name == db.sequelize.models.datos_facturacion_domicilios.name){
						let where = {}
...
async function restaurar(req,res){
	...
	try {
		const registroARestaurar = await db.sequelize.models.datos_facturacion_domicilios.findByPk(id,{ paranoid: false });
				...
				const registrosEncontrados = await db.sequelize.models.datos_facturacion_domicilios.findAll({
					where: {
						...
						[db.Sequelize.Op.or]: [
							...
							{
								[db.Sequelize.Op.and]: [
									{ id_dato_facturacion: registroARestaurar.id_dato_facturacion },
									{ tipo: 'F' }
								]
							}
						]
				...
				if(registrosEncontrados.length > 0){
					...
					await registrosEncontrados.forEach(registro => {                     
                        if((registro.id_domicilio == (registroARestaurar.id_domicilio)) || 
						   (registro.id_dato_facturacion == (registroARestaurar.id_dato_facturacion) && ((registroARestaurar.tipo == "F") && registro.tipo == "F")) &&
						    registro.id != id){
							if(!regExistente){
								regExistente = true;
								res.status(400).send({ status: false, msg: "Registro existente"});
