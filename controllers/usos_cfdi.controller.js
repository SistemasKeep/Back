...
async function index(req, res) {
	...
	const camposModelo = Object.keys(db.sequelize.models.usos_cfdi.rawAttributes);
	...
	try {
		...
		const docs = await db.sequelize.models.usos_cfdi.findAll({
			paranoid: false,
			include: relaciones,
			page: page || 1,
		...
		const dataDocs = await db.sequelize.models.usos_cfdi.count({
			paranoid: false,
			where: filtro
		});
		...
		const fullUrl = `${req.protocol}://${req.get('host')}/api/usosCfdi`;
...
async function store(req, res){
	...
	try {
		...
		const registrosEncontrados = await db.sequelize.models.usos_cfdi.findAll({
			where: {
				[db.Sequelize.Op.and]: {
					deletedAt: null
				},
		...
		const nuevoRegistro = await db.sequelize.models.usos_cfdi.create(registro);
...
async function show(req, res){
	...
	try {
		const findRelaciones = new Relaciones([],[],db.sequelize.models)
		const relaciones = await findRelaciones.getRelaciones()
		const registroEncontrado = await db.sequelize.models.usos_cfdi.findByPk(id,{ paranoid: false,include: relaciones });
...
async function update(req, res){
	...
	try {
		...
		const registroAEditar = await db.sequelize.models.usos_cfdi.findByPk(id);
		...
		const registrosEncontrados = await db.sequelize.models.usos_cfdi.findAll(whereFind);
		if(registrosEncontrados.length > 0){
			var regExistente = false
			await registrosEncontrados.forEach(registro => {
...
async function destroy(req,res){
	...
	try {
		const registroAEliminar = await db.sequelize.models.usos_cfdi.findByPk(id);
		if(registroAEliminar != null){
			...
			for (const modelo of Object.values(db.sequelize.models)) {
				...
				for (const asociacion of Object.values(asociaciones)) {
					if(asociacion.target.name == db.sequelize.models.usos_cfdi.name){
						let where = {}
...
async function restaurar(req,res){
	...
	try {
		const registroARestaurar = await db.sequelize.models.usos_cfdi.findByPk(id,{ paranoid: false });
