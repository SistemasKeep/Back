...
async function index(req, res) {
	...
	const camposModelo = Object.keys(db.sequelize.models.commodity_categorias.rawAttributes);
	...
	try {
		...
		const docs = await db.sequelize.models.commodity_categorias.findAll({
			paranoid: false,
			page: page || 1,
			include: relaciones,
		...
		const dataDocs = await db.sequelize.models.commodity_categorias.count({
			paranoid: false,
			include: relaciones,
			where: filtro
		});
		...
		const fullUrl = `${req.protocol}://${req.get('host')}/api/commodityCategorias`;
...
async function store(req, res){
	...
	try {
		...
		const registrosEncontrados = await db.sequelize.models.commodity_categorias.findAll({
			where: {
				descripcion: {
		...
		const nuevoRegistro = await db.sequelize.models.commodity_categorias.create(registro);
...
async function show(req, res){
	...
	try {
		const findRelaciones = new Relaciones([],[],db.sequelize.models)
		const relaciones = await findRelaciones.getRelaciones()
		const registroEncontrado = await db.sequelize.models.commodity_categorias.findByPk(id,{ include: relaciones, paranoid: false });
...
async function update(req, res){
	...
	try {
		...
		const registroAEditar = await db.sequelize.models.commodity_categorias.findByPk(id);
		...
		const registrosEncontrados = await db.sequelize.models.commodity_categorias.findAll(whereFind);
		if(registrosEncontrados.length > 0){
			var regExistente = false
			await registrosEncontrados.forEach(registro => {
		...
		var registro2 = {
			...
			tabla: db.sequelize.models.commodity_categorias.name.toUpperCase(),
			accion: 'EDICION',
			createdAt: moment().tz('America/Mexico_City')
		}
...
async function destroy(req,res){
	...
	try {
		const registroAEliminar = await db.sequelize.models.commodity_categorias.findByPk(id);
		if(registroAEliminar != null){
			...
			for (const modelo of Object.values(db.sequelize.models)) {
				...
				for (const asociacion of Object.values(asociaciones)) {
					if(asociacion.target.name == db.sequelize.models.commodity_categorias.name){
						let where = {}
...
async function restaurar(req,res){
	...
	try {
		const registroARestaurar = await db.sequelize.models.commodity_categorias.findByPk(id,{ paranoid: false });
...
async function showHistoricos(req, res) {
	...
	if(registro.tabla != db.sequelize.models.commodity_categorias.name.toUpperCase()){
		return res.status(400).send({ status: false, msg: "Registro no pertenece al crud commodity_categorias" });
	} 
	let usuario = await db.sequelize.models.usuarios.findByPk(registro.id_usuario_registro);
	reg.id = registro.id;
	reg.usuario_registro = {id: usuario.id, nombre: usuario.nombre};
...
async function exportar(req, res) {
	...
	try {
		...
		const nombreReporte = 'Categorías de Commoditie';
		const namesSheets = [db.sequelize.models.commodity_categorias.name];
