...
async function index(req, res) {
	...
	const camposModelo = Object.keys(db.sequelize.models.razones_bloqueo.rawAttributes);
	...
	try {
		...
		const docs = await db.sequelize.models.razones_bloqueo.findAll({
			paranoid: false,
			include: relaciones,
			page: page || 1,
		...
		const dataDocs = await db.sequelize.models.razones_bloqueo.count({
		    paranoid: false,
			include: relaciones,
		    where: filtro
		});
		...
		const fullUrl = `${req.protocol}://${req.get('host')}/api/razonesBloqueo`;
...
async function store(req, res){
	...
	try {
		...
		const registrosEncontrados = await db.sequelize.models.razones_bloqueo.findAll({
			where: {
				[db.Sequelize.Op.and]: {
					deletedAt: null
				},
		...
		const nuevoRegistro = await db.sequelize.models.razones_bloqueo.create(registro);
...
async function show(req, res){
	...
	try {
		const registroEncontrado = await db.sequelize.models.razones_bloqueo.findByPk(id,{ paranoid: false });
...
async function update(req, res){
	...
	try {
		...
		const registroAEditar = await db.sequelize.models.razones_bloqueo.findByPk(id);
		...
		const registrosEncontrados = await db.sequelize.models.razones_bloqueo.findAll(whereFind);
		if(registrosEncontrados.length > 0){
			var regExistente = false
			await registrosEncontrados.forEach(registro => {
...
async function destroy(req,res){
	...
	try {
		const registroAEliminar = await db.sequelize.models.razones_bloqueo.findByPk(id);
		if(registroAEliminar != null){
			...
			for (const modelo of Object.values(db.sequelize.models)) {
				...
				for (const asociacion of Object.values(asociaciones)) {
					if(asociacion.target.name == db.sequelize.models.razones_bloqueo.name){
						let where = {}
...
async function restaurar(req,res){
	...
	try {
		const registroARestaurar = await db.sequelize.models.razones_bloqueo.findByPk(id,{ paranoid: false });
