...
async function index(req, res) {
	...
	const camposModelo = Object.keys(db.sequelize.models.productos_unidades_medida.rawAttributes);
	...
	try {
		...
		const docs = await db.sequelize.models.productos_unidades_medida.findAll({
			paranoid: false,
			include: relaciones,
			page: page || 1,
		...
		const dataDocs = await db.sequelize.models.productos_unidades_medida.count({
		    paranoid: false,
		    where: filtro
		});
		...
		const fullUrl = `${req.protocol}://${req.get('host')}/api/productosUnidadesMedida`;
...
async function store(req, res){
	try {
		...
		let obligatorios = [{campo:'clave', tipo:'string', largo:5, textoCase:"up"},
                            {campo:'nombre', tipo:'string', largo:45}
        ]
		registro = await Validaciones.validParametros(req, res,obligatorios,registro);
		if(!registro){
			return '';
		}
        const validosOpcionales =[{campo:'claveUnidadMedidaSat', tipo:'string', largo:20},
		                          {campo:'unidadMedidaSat', tipo:'string', largo:100, textoCase:"up"},
								  {campo:'cantidadSeleccionable', tipo:'boolean'},
        ]
		...
		const registrosEncontrados = await db.sequelize.models.productos_unidades_medida.findAll({
			where: {
				[db.Sequelize.Op.or]: {
                    clave: {
					...
					clave_unidad_medida_sat: {
						[db.Sequelize.Op.like]: `%${parametros.claveUnidadMedidaSat}%`
					}
				},
				deletedAt: null
			}
		...
		if(registrosEncontrados.length > 0){
			...
			await registrosEncontrados.forEach(registro => {
				if((registro.clave.toLowerCase() == parametros.clave.toLowerCase()) ||
                   registro.clave_unidad_medida_sat.toLowerCase() == parametros.claveUnidadMedidaSat.toLowerCase()){
					if(!regExistente){
						regExistente = true;
		...
		const nuevoRegistro = await db.sequelize.models.productos_unidades_medida.create(registro);
...
async function show(req, res){
	...
	try {
		const registroEncontrado = await db.sequelize.models.productos_unidades_medida.findByPk(id, {paranoid: false});
...
async function update(req, res){
	...
	try {
		...
		const validosOpcionales = [{campo:'claveUnidadMedidaSat', tipo:'string', largo:20},
		                           {campo:'unidadMedidaSat', tipo:'string', largo:100, textoCase:"up"},
		                           {campo:'cantidadSeleccionable', tipo:'boolean'},
		                           {campo:'clave', tipo:'string', largo:5, textoCase:"up"},
		                           {campo:'nombre', tipo:'string', largo:45}
        ]
		...
		const registroAEditar = await db.sequelize.models.productos_unidades_medida.findByPk(id);
		...
		var whereFind = {
			where: {
				[db.Sequelize.Op.or]: {
					...
					clave_unidad_medida_sat: {
						[db.Sequelize.Op.like]: `%${parametros.claveUnidadMedidaSat != undefined ? parametros.claveUnidadMedidaSat :registroAEditar.clave_unidad_medida_sat}%`
					}
				},
				deletedAt: null
			}
		}
		const registrosEncontrados = await db.sequelize.models.productos_unidades_medida.findAll(whereFind);
		if(registrosEncontrados.length > 0){
			var regExistente = false
			await registrosEncontrados.forEach(registro => {
				if(((registro.clave.toLowerCase() == (parametros.clave != undefined? parametros.clave.toLowerCase(): registroAEditar.clave.toLowerCase())) ||
                   registro.clave_unidad_medida_sat.toLowerCase() == (parametros.claveUnidadMedidaSat != undefined? parametros.claveUnidadMedidaSat.toLowerCase(): registroAEditar.clave_unidad_medida_sat.toLowerCase())) &&
				   registro.id != id){
					if(!regExistente){
						regExistente = true;
						res.status(400).send({ status: false, msg: "Registro existente"});
...
async function destroy(req,res){
	...
	try {
		const registroAEliminar = await db.sequelize.models.productos_unidades_medida.findByPk(id);
		if(registroAEliminar != null){
			...
			for (const modelo of Object.values(db.sequelize.models)) {
				...
				for (const asociacion of Object.values(asociaciones)) {
					if(asociacion.target.name == db.sequelize.models.productos_unidades_medida.name){
						let where = {}
...
async function restaurar(req,res){
	...
	try {
		const registroARestaurar = await db.sequelize.models.productos_unidades_medida.findByPk(id,{ paranoid: false });
			...
			if(registroARestaurar.deletedAt != null){
				const registrosEncontrados = await db.sequelize.models.productos_unidades_medida.findAll({
					where: {
						[db.Sequelize.Op.or]: {
							clave_unidad_medida_sat: {
								[db.Sequelize.Op.like]: `%${registroARestaurar.clave_unidad_medida_sat}%`
							},
							clave: {
				...
				if(registrosEncontrados.length > 0){
					var regExistente = false
					await registrosEncontrados.forEach(registro => {
						if(((registro.clave_unidad_medida_sat.toLowerCase() == registroARestaurar.clave_unidad_medida_sat.toLowerCase()) ||
