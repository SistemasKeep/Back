...
async function index(req, res) {
	...
	try {
		...
		if(perfilesValidos.includes(req.query.perfil)){
			...
			try {
				const relacionesValidas = [ 'nacionalidad', 'nacionalidad.continente' ]
		...
		const dataDocs = await db.sequelize.models.puertos_aeropuertos.count({
		    paranoid: false,
		    include: relaciones,
		    where: filtro
		});
		...
		const fullUrl = `${req.protocol}://${req.get('host')}/api/puertosAeropuertos`;
...
async function store(req, res){
	...
	let obligatorios = [{campo:'idNacionalidad', tipo:'model',model:db.sequelize.models.paises},
						...
						{campo:'tipo', tipo:'boolean'}]
	registro = await Validaciones.validParametros(req, res,obligatorios,registro);
	if(!registro){
		return '';
	}
	try {
		const registrosEncontrados = await db.sequelize.models.puertos_aeropuertos.findAll({
			where: {
		...
		const nuevoRegistro = await db.sequelize.models.puertos_aeropuertos.create(registro);
...
async function show(req, res){
	...
	try {
		...
		const registroEncontrado = await db.sequelize.models.puertos_aeropuertos.findByPk(id,{include:relaciones,paranoid: false});
...
async function update(req, res){
	...
	try {
		const registroAEditar = await db.sequelize.models.puertos_aeropuertos.findByPk(id);
		...
		var whereFind = {
			where:{
				tipo: parametros.tipo != undefined ? parametros.tipo : registroAEditar.tipo,
				descripcion: {
		...
		const registrosEncontrados = await db.sequelize.models.puertos_aeropuertos.findAll(whereFind);
		if(registrosEncontrados.length > 0){
			var regExistente = false
			await registrosEncontrados.forEach(registro => {
				if(((registro.descripcion.toLowerCase() == (parametros.descripcion != undefined ? parametros.descripcion.toLowerCase() : registroAEditar.descripcion.toLowerCase())) && 
				   registro.tipo == parametros.tipo != undefined ? parametros.tipo : registroAEditar.tipo) &&
				   registro.id != id){
					if(!regExistente){
						regExistente = true;
						res.status(400).send({ status: false, msg: "Registro existente"});
		...
		var registro2 = {
			...
			tabla: db.sequelize.models.puertos_aeropuertos.name.toUpperCase(),
			accion: 'EDICION',
			createdAt: moment().tz('America/Mexico_City')
		}
...
async function destroy(req,res){
	...
	try {
		const registroAEliminar = await db.sequelize.models.puertos_aeropuertos.findByPk(id);
		if(registroAEliminar != null){
			...
			for (const modelo of Object.values(db.sequelize.models)) {
				...
				for (const asociacion of Object.values(asociaciones)) {
					if(asociacion.target.name == db.sequelize.models.puertos_aeropuertos.name){
						let where = {}
...
async function restaurar(req,res){
	...
	try {
		const registroARestaurar = await db.sequelize.models.puertos_aeropuertos.findByPk(id,{ paranoid: false });
		if(registroARestaurar != null){
			if(registroARestaurar.deletedAt != null){
				const registrosEncontrados = await db.sequelize.models.puertos_aeropuertos.findAll({
					where: {
						tipo: {
							[db.Sequelize.Op.like]: registroARestaurar.tipo
						},
						descripcion: {
				...
				if(registrosEncontrados.length > 0){
					...
					await registrosEncontrados.forEach(registro => {
						if((registro.descripcion.toLowerCase() == registroARestaurar.descripcion.toLowerCase() || 
						   registro.tipo == registroARestaurar.tipo) &&
						   registro.id != id){
							if(!regExistente){
								regExistente = true;
								res.status(400).send({ status: false, msg: "Registro existente"});
...
async function showHistoricos(req, res) {
	...
	if(registro.tabla != db.sequelize.models.puertos_aeropuertos.name.toUpperCase()){
		return res.status(400).send({ status: false, msg: "Registro no pertenece al crud puertos_aeropuertos" });
	} 
	let usuario = await db.sequelize.models.usuarios.findByPk(registro.id_usuario_registro);
	reg.id = registro.id;
	reg.usuario_registro = {id: usuario.id, nombre: usuario.nombre};
