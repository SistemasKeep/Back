	...
	try {
		...
		for(const doc of docs){
			...
			if(req.query.perfil  == 'all'){
				if(element.id_certificado !== null){
				...
				const facturaDetalle = await db.sequelize.models.factura_detalles.findAll({where:{id_pedido_factura: element.id}, include:relacionesFacDet})
				if(facturaDetalle.length > 0){
					element.factura_detallesList = []
					element.impuestoPedido = 0
					element.descuentoPedido = 0
					element.subtotalPedido = 0
					element.importePedido = 0
					for(const detalles of facturaDetalle){
						const factuDealle = detalles.toJSON()
						if(factuDealle.factura != null){
							const listRelPagos = [ 
								'pagos.pago.cfdi',
							...
							const cxcAux = await db.sequelize.models.cuentas_por_cobrar.findOne({where:{id_factura:factuDealle.id_factura}})
							factuDealle.factura.cxc = null
							factuDealle.factura.factura_pagada = false
							if(cxcAux !== null){
								const cxc = await db.sequelize.models.cuentas_por_cobrar.findByPk(cxcAux.id,{ include:relacionesPagos})
								factuDealle.factura.cxc = cxc
								factuDealle.factura.factura_pagada = parseFloat(cxc.saldo) == 0
							}
							...
							const clienteRazonSocial = await db.sequelize.models.clientes_razones_sociales.findOne({where:{id_razon_social: factuDealle.factura.id_razon_social}, include:relacionescliente})
							let fechaVencimiento = null
							let diferenciaFechas = null
							if(factuDealle.factura.cxc != null){
								fechaVencimiento = moment(factuDealle.factura.cxc.fecha_vencimiento).tz('America/Mexico_City');
							...
							element.impuestoPedido = element.impuestoPedido + parseFloat((impuestoFactura).toFixed(2))
							element.descuentoPedido = element.descuentoPedido + parseFloat((descuentoFactura).toFixed(2))
							element.subtotalPedido = element.subtotalPedido + subtotalFactura
							element.importePedido = element.importePedido + parseFloat((subtotalFactura - descuentoFactura + parseFloat((impuestoFactura).toFixed(2))).toFixed(2))
							const totalFactura = (subtotalFactura - descuentoFactura + parseFloat((impuestoFactura).toFixed(2))).toFixed(4)
	
							//Se obtiene el tipo de cambio del dia
							let fechaString = moment(factuDealle.factura.createdAt).tz('America/Mexico_City').format('YYYY-MM-DD')
							...
							let fechaAplicacion = ''
							if(factuDealle.factura.cxc != null){
								for(const pago of factuDealle.factura.cxc.pagos){
							...
							const idMarca = factuDealle.factura.id_marca
							...
							if(agentesCliente == null){
								const marcaId = idMarca == 2 ? 2 : idMarca == 1 ? 1 : null
								agentesCliente = await db.sequelize.models.marca_agentes_clientes.findOne({where:{id_cliente:clienteRazonSocial.cliente.id, id_marca: marcaId}, include:relacionesAgente,paranoid: false})
							}
							...
							factuDealle.factura.cliente = cliente
							factuDealle.factura.tipo_cambio = tipoCambio
							factuDealle.factura.folio_ingreso = folioPago != '' ? folioPago : null
							factuDealle.factura.fecha_pago = fechaPago != '' ? fechaPago : null
							factuDealle.factura.fecha_aplicacion_ingreso = fechaAplicacion != '' ? fechaAplicacion : null
							factuDealle.factura.agente_credito_cobranza = clienteDetalles.agente_credito_cobranza !== null && clienteDetalles.agente_credito_cobranza !== undefined ? clienteDetalles.agente_credito_cobranza: null
							factuDealle.factura.agente_operativo = agentesCliente !== null && agentesCliente !== undefined ? agentesCliente.agente_operativo : null
							factuDealle.factura.agente_venta = agentesCliente !== null && agentesCliente !== undefined ? agentesCliente.agente_venta_1 : null
							factuDealle.factura.notas_credito = await db.sequelize.models.notas_credito.findAll({where:{id_factura:factuDealle.factura.id}}),
							factuDealle.factura.total_factura = parseFloat(parseFloat(totalFactura).toFixed(2))
							factuDealle.factura.saldo_saldado = parseFloat((totalFactura - (factuDealle.factura.cxc != null ? factuDealle.factura.cxc.saldo : 0)).toFixed(2)) 
						}
						element.factura_detallesList.push(factuDealle)
					}
					element.factura_detalles = element.factura_detallesList[0]
				} else{
					if(element.id_certificado !== null){
						...
						element.subtotalPedido = subtotalFactura
						element.importePedido = parseFloat((subtotalFactura - descuentoFactura + parseFloat((impuestoFactura).toFixed(2))).toFixed(4))
						element.factura_detalles = null
					}else if(element.id_servicio_ontrack !== null){
						const servicioMonitoreo = await db.sequelize.models.servicios_ontrack.findByPk(element.id_servicio_ontrack, { include:['servicios_ontrack_detalles'], paranoid: false });
						let subtotal = 0
						let descuento = 0
						let impuesto = 0
						for(const detalle of servicioMonitoreo.servicios_ontrack_detalles){
							subtotal = subtotal + parseFloat(detalle.subtotal) * parseInt(detalle.cantidad ?? 1)
							descuento = descuento + parseFloat(detalle.descuento_monto)
							impuesto = impuesto + parseFloat(detalle.monto_iva)
						}
						element.impuestoPedido = parseFloat((impuesto).toFixed(2))
						element.descuentoPedido = parseFloat((descuento).toFixed(2))
						element.subtotalPedido = subtotal
						element.importePedido = parseFloat((subtotal - descuento + parseFloat((impuesto).toFixed(2))).toFixed(4))
						element.factura_detalles = null
					}
				}
				if(element.id_certificado !== null){
					element.marca = element.certificado.marca;
					...
					element.marca = element.servicios_ontrack.marca;
					const moneda = await db.sequelize.models.monedas.findByPk(element.servicios_ontrack.id_moneda);
					element.moneda = moneda != null ? moneda : null;
					if(element.servicios_ontrack.oficina_razon_social != null){
						const razonSocial = await db.sequelize.models.razones_sociales.findByPk(element.servicios_ontrack.oficina_razon_social.id_razon_social);
						element.razon_social = razonSocial != null ? razonSocial : null;
					}else{
						element.razon_social = null;
					}
...
async function exportacion(req, res) {
    ...
    try {
		...
		if(docs.length < 1){
            return res.status(400).json({ status: false, error: 'Sin registros' });
        }
		if(docs.length > 0){
        ...
        for(const element of data){
			...
			const cliente = await db.sequelize.models.clientes.findByPk(element.certificado !== null ? element.certificado.id_cliente : element.servicios_ontrack !== null ? element.servicios_ontrack.id_cliente : null);
			...
			const marca = await db.sequelize.models.marcas.findByPk(element.certificado !== null ? element.certificado.id_marca : element.servicios_ontrack !== null ? element.servicios_ontrack.id_marca : null);
			const moneda = await db.sequelize.models.monedas.findByPk(element.certificado !== null ? element.certificado.id_moneda : element.servicios_ontrack !== null ? element.servicios_ontrack.id_moneda : null);
			//Se obtiene el tipo de cambio del dia
			...
			if(element.factura_detalles == null){
				const folio = element.id
				let referencia = ""
				if(element.certificado !== null){
					referencia = element.certificado.no_operacion !== null ? element.certificado.no_operacion : ""
				} else if(element.servicios_ontrack !== null){
					referencia = element.servicios_ontrack.no_operacion !== null ? element.servicios_ontrack.no_operacion : ""
				}
				elementos.push({
					'Folio': folio,
			...
			const valorUnitario = parseFloat( element.factura_detalles == null ? 0 : (element.factura_detalles.precio_unitario ?? 0));
			const descuentoGeneral = parseFloat(element.factura_detalles == null ? 0 : (element.factura_detalles.descuento ?? 0));
			const impuesto = parseFloat(element.factura_detalles == null ? 0 :  (element.factura_detalles.impuesto ?? 0));
			...
			elementos.push({
				'Folio': element.factura_detalles == null ? "" : element.factura_detalles.factura.folio,
				'Referencia':element.factura_detalles == null ? "" : element.factura_detalles.factura.referencia,
				'Fecha': moment(element.createdAt).tz('America/Mexico_City').format('YYYY-MM-DD'),
				'Facturado':element.factura_detalles != null ? "Si" : "No",
                'Factura pagada': element.factura_detalles == null ? "" : (element.factura_detalles.factura.factura_pagada ? 'Si' : 'No'),
                'Día de Facturación': element.factura_detalles == null ? "" : moment(element.factura_detalles.factura.createdAt).tz('America/Mexico_City').format('YYYY-MM-DD'),
                'Cliente': cliente.nombre,
                'Persona que Solicita': element.usuario_registro.nombre,
				'Marca': marca.nombre,
				'Impuesto': impuesto,
				...
				'Moneda': moneda.clave,
				'Tipo de Cambio': tipoCambio,
				'Saldado': element.factura_detalles == null ? "" : (element.factura_detalles.factura.factura_pagada ? 'Si' : 'No'),
				'Ejecutivo de Cobranza':clienteDetalles.agente_credito_cobranza !== null && clienteDetalles.agente_credito_cobranza !== undefined ? clienteDetalles.agente_credito_cobranza.nombre: '',
