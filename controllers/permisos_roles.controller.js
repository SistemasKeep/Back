...
async function index(req, res) {
	...
	const camposModelo = Object.keys(db.sequelize.models.permisos_roles.rawAttributes);
	...
	try {
		const perfilesValidos = ['permiso', 'rol', 'all']
		var relaciones = []
		if(perfilesValidos.includes(req.query.perfil)){
			...
			try {
				const relacionesValidas = ['permiso', 'rol']
		...
		const docs = await db.sequelize.models.permisos_roles.findAll({
			paranoid: false,
			page: page || 1,
			include: relaciones,
		...
		const dataDocs = await db.sequelize.models.permisos_roles.count({
		    paranoid: false,
		    include: relaciones,
		    where: filtro
		});		
		...
		const fullUrl = `${req.protocol}://${req.get('host')}/api/permisosRoles`;
...
async function store(req, res){
	try {
		...
		let obligatorios = [
            {campo:'idRole', tipo:'model', model:db.sequelize.models.roles},
            {campo:'idPermiso', tipo:'model', model:db.sequelize.models.permisos}
		]
		registro = await Validaciones.validParametros(req, res,obligatorios,registro);
		if(!registro){
			return '';
		}
		const registrosEncontrados = await db.sequelize.models.permisos_roles.findAll({
			where: {
				[db.Sequelize.Op.and]: {
					id_role: parametros.idRole,
                    id_permiso: parametros.idPermiso
				}
			}
		});
		if(registrosEncontrados.length > 0){
			...
			await registrosEncontrados.forEach(registro => {
				if(registro.id_role == parametros.idRole || registro.id_permiso == parametros.idPermiso){
					regExistente = true;
					res.status(400).send({ status: false, msg: "Ese rol ya ha sido registrado para este usuario"});
				}
			});
			if(regExistente){
				return '';
		...
		const nuevoRegistro = await db.sequelize.models.permisos_roles.create(registro);
...
async function show(req, res){
	...
	try {
		...
		const registroEncontrado = await db.sequelize.models.permisos_roles.findByPk(id,{include:relaciones,paranoid: false});
...
async function update(req, res){
	...
	try {
		...
		const registroAEditar = await db.sequelize.models.permisos_roles.findByPk(id);
		...
		var whereFind = {
			where: {
				[db.Sequelize.Op.and]: {
					id_role: parametros.idRole != undefined ? parametros.idRole : registroAEditar.id_role,
                    id_permiso: parametros.idPermiso != undefined ? parametros.idPermiso : registroAEditar.id_permiso,
                    deletedAt: null
				}
			}
		}
		const registrosEncontrados = await db.sequelize.models.permisos_roles.findAll(whereFind);
		if(registrosEncontrados.length > 0){
			var regExistente = false
			await registrosEncontrados.forEach(registro => {
				if((registro.id_role == (parametros.idRole != undefined ? parametros.idRole : registroAEditar.id_role) &&
                    registro.id_permiso == (parametros.idPermiso != undefined ? parametros.idPermiso : registroAEditar.id_permiso)) && 
                    registro.id != id){
						regExistente = true;
						res.status(400).send({ status: false, msg: "Ese rol ya ha sido registrado para este usuario"});
			        }
...
async function destroy(req,res){
	...
	try {
		const registroAEliminar = await db.sequelize.models.permisos_roles.findByPk(id);
		if(registroAEliminar != null){
			...
			for (const modelo of Object.values(db.sequelize.models)) {
				...
				for (const asociacion of Object.values(asociaciones)) {
					if(asociacion.target.name == db.sequelize.models.permisos_roles.name){
						let where = {}
...
async function restaurar(req,res){
	...
	try {
		const registroARestaurar = await db.sequelize.models.permisos_roles.findByPk(id,{ paranoid: false });
		if(registroARestaurar != null){
			if(registroARestaurar.deletedAt != null){
				const registrosEncontrados = await db.sequelize.models.permisos_roles.findAll({
					where: {
						[db.Sequelize.Op.and]: {
							id_role: registroARestaurar.id_role,
                            id_permiso: registroARestaurar.id_permiso,
							deletedAt: null
						}
					}
				});
				...
				if(registrosEncontrados.length > 0){
					var regExistente = false
					await registrosEncontrados.forEach(registro => {
						if((registro.id_role == registroARestaurar.id_role && registro.id_permiso == registroARestaurar.id_permiso) &&
							registro.id != id){
								regExistente = true;
