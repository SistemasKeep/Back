...
async function index(req, res) {
	...
	const camposModelo = Object.keys(db.sequelize.models.conceptos_presupuesto.rawAttributes);
	...
	try {
		...
		const docs = await db.sequelize.models.conceptos_presupuesto.findAll({
			paranoid: false,
			page: page || 1,
			include: relaciones,
		...
		const dataDocs = await db.sequelize.models.conceptos_presupuesto.count({
			paranoid: false,
			include: relaciones,
			where: filtro
		});
		...
		const fullUrl = `${req.protocol}://${req.get('host')}/api/conceptosPresupuesto`;
		...
		for(const doc of docs){
			...
			if(req.query.perfil == 'all'){
				element.patidas_hijos = await db.sequelize.models.conceptos_presupuesto.findAll({where:{id_partida_padre:element.id}})
			}
			data.push(element)
		}
...
async function store(req, res){
	...
	try {
		...
		const validosOpcionales =[{campo:'idPartidaPadre', tipo:'model',model:db.sequelize.models.conceptos_presupuesto}]
		...
		const registrosEncontrados = await db.sequelize.models.conceptos_presupuesto.findAll({
			where: {
				nombre: {
		...
		const nuevoRegistro = await db.sequelize.models.conceptos_presupuesto.create(registro);
...
async function show(req, res){
	...
	try {
		...
		const registroEncontrado = await db.sequelize.models.conceptos_presupuesto.findByPk(id,{include:relaciones,paranoid: false});
...
async function update(req, res){
	...
	try {
		...
		const validosOpcionales = [{campo:'descripcion', tipo:'string',largo:255,textoCase:"up"},
								   {campo:'nombre', tipo:'string',largo:100,textoCase:"up"},
								   {campo:'idPartidaPadre', tipo:'model',model:db.sequelize.models.conceptos_presupuesto}]
		...
		const registroAEditar = await db.sequelize.models.conceptos_presupuesto.findByPk(id);
		...
		const registrosEncontrados = await db.sequelize.models.conceptos_presupuesto.findAll(whereFind);
		if(registrosEncontrados.length > 0){
			var regExistente = false
			await registrosEncontrados.forEach(registro => {
		...
		var registro2 = {
			...
			tabla: db.sequelize.models.conceptos_presupuesto.name.toUpperCase() ,
			accion: 'EDICION',
			createdAt: moment().tz('America/Mexico_City')
		}
...
async function destroy(req,res){
	...
	try {
		const registroAEliminar = await db.sequelize.models.conceptos_presupuesto.findByPk(id);
		if(registroAEliminar != null){
			...
			for (const modelo of Object.values(db.sequelize.models)) {
				...
				for (const asociacion of Object.values(asociaciones)) {
					if(asociacion.target.name == db.sequelize.models.conceptos_presupuesto.name){
						let where = {}
...
async function restaurar(req,res){
	...
	try {
		const registroARestaurar = await db.sequelize.models.conceptos_presupuesto.findByPk(id,{ paranoid: false });
...
async function showHistoricos(req, res) {
	...
	if(registro.tabla != db.sequelize.models.conceptos_presupuesto.name.toUpperCase()){
		return res.status(400).send({ status: false, msg: "Registro no pertenece al crud conceptos_presupuesto" });
	} 
	let usuario = await db.sequelize.models.usuarios.findByPk(registro.id_usuario_registro);
	reg.id = registro.id;
	reg.usuario_registro = {id: usuario.id, nombre: usuario.nombre};
...
async function exportacion(req, res) {
    ...
    try {
        ...
        const nombreReporte = `conceptos_presupuesto_${moment().tz('America/Mexico_City').format('YYYY-MM-DD')}`;
        const namesSheets = [db.sequelize.models.conceptos_presupuesto.name];
        const reporteConceptosPresupuesto = new ReportesXLSX({
            nombreReporte:nombreReporte,
            elementos:elementos,
            namesSheets:namesSheets, 
            idMarca:idMarca
        });
        return await reporteConceptosPresupuesto.gerReporteOneSheet(res,req);
    } catch (error) {
        return res.status(500).json({ success: false, error: 'Error interno del servidor', error: error.toString() });
    }
