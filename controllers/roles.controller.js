...
async function index(req, res) {
	...
	const camposModelo = Object.keys(db.sequelize.models.roles.rawAttributes);
	...
	try {
		...
		const docs = await db.sequelize.models.roles.findAll({
			paranoid: false,
			include: relaciones,
			page: page || 1,
		...
		const dataDocs = await db.sequelize.models.roles.count({
			paranoid: false,
			include: relaciones,
			where: filtro
		});
		...
		const fullUrl = `${req.protocol}://${req.get('host')}/api/roles`;
...
async function store(req, res){
	try {
		...
		let obligatorios = [
            ...
            {campo:'reservado', tipo:'boolean'}
		]
		registro = await Validaciones.validParametros(req, res,obligatorios,registro);
		if(!registro){
			return '';
		}
		const registrosEncontrados = await db.sequelize.models.roles.findAll({
			where: {
				[db.Sequelize.Op.or]: {
					name: {
                    ...
                    display_name: {
						[db.Sequelize.Op.like]: `%${parametros.displayName}%`
					}
				}
			}
		});
		if(registrosEncontrados.length > 0){
			...
			await registrosEncontrados.forEach(registro => {
				if(registro.name.toLowerCase() == parametros.name.toLowerCase() || registro.display_name.toLowerCase() == parametros.displayName.toLowerCase()){
					regExistente = true;
					res.status(400).send({ status: false, msg: "El nombre o clave del rol ya se encuentra registrado"});
				}
			});
			if(regExistente){
				return '';
		...
		const nuevoRegistro = await db.sequelize.models.roles.create(registro);
...
async function show(req, res){
	...
	try {
		...
		const registroEncontrado = await db.sequelize.models.roles.findByPk(id,{include:relaciones,paranoid: false});
...
async function update(req, res){
	...
	try {
		...
		const registroAEditar = await db.sequelize.models.roles.findByPk(id);
		...
		var whereFind = {
			where: {
				[db.Sequelize.Op.or]: {
                    ...
                    display_name: {
						[db.Sequelize.Op.like]: `%${parametros.displayName != undefined ? parametros.displayName : registroAEditar.display_name}%`
					},
				},
				[db.Sequelize.Op.and]: {
					deletedAt: null
		...
		const registrosEncontrados = await db.sequelize.models.roles.findAll(whereFind);
		if(registrosEncontrados.length > 0){
			var regExistente = false
			await registrosEncontrados.forEach(registro => {
				if((registro.name.toLowerCase() == (parametros.name != undefined ? parametros.name.toLowerCase() : registroAEditar.name.toLowerCase()) ||
                    registro.display_name.toLowerCase() == (parametros.displayName != undefined ? parametros.displayName.toLowerCase() : registroAEditar.display_name.toLowerCase())) && 
                    registro.id != id)
                    {
						regExistente = true;
						res.status(400).send({ status: false, msg: "El nombre o clave del rol ya se encuentra registrado"});
...
async function destroy(req,res){
	...
	try {
		const registroAEliminar = await db.sequelize.models.roles.findByPk(id);
		if(registroAEliminar != null){
			...
			for (const modelo of Object.values(db.sequelize.models)) {
				...
				for (const asociacion of Object.values(asociaciones)) {
					if(asociacion.target.name == db.sequelize.models.roles.name){
						let where = {}
...
async function restaurar(req,res){
	...
	try {
		const registroARestaurar = await db.sequelize.models.roles.findByPk(id,{ paranoid: false });
			...
			if(registroARestaurar.deletedAt != null){
				const registrosEncontrados = await db.sequelize.models.roles.findAll({
					where: {
						[db.Sequelize.Op.and]: {
							name: {
								[db.Sequelize.Op.like]: `%${registroARestaurar.name}%`
							},
							display_name: {
								[db.Sequelize.Op.like]: `%${registroARestaurar.display_name}%`
							},
							deletedAt: null
						}
					}
				...
				if(registrosEncontrados.length > 0){
					var regExistente = false
					await registrosEncontrados.forEach(registro => {
						if((registro.name == registroARestaurar.name && registro.display_name == registroARestaurar.display_name) &&
							registro.id != id){
								regExistente = true;
...
async function exportar(req, res) {
	...
	try {
		...
		for (let i = 0; i < docs.length; i++) {
			...
			aux = {
				'Clave': elemento.display_name,
				'Nombre': elemento.name,
				'Descripción': elemento.descripcion,
				'Reservado': elemento.reservado == true ? 'Si' : 'No',
			};
			dataExcel.push(aux);
		}
		...
		const nombreReporte = 'Roles';
		const namesSheets = [db.sequelize.models.roles.name];
