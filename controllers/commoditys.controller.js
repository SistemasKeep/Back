...
async function index(req, res) {
	...
	try {
		...
		const dataDocs = await db.sequelize.models.commoditys.count({
			paranoid: false,
			include: relaciones,
			where: filtro
		});
		...
		const fullUrl = `${req.protocol}://${req.get('host')}/api/commoditys`;
...
async function store(req, res){
	...
	try {
		...
		let obligatorios = [
			{campo:'idCommodityCategoria', tipo:'model',model:db.sequelize.models.commodity_categorias},
		...
		const registrosEncontrados = await db.sequelize.models.commoditys.findAll({
			where: {
				descripcion: {
		...
		const nuevoRegistro = await db.sequelize.models.commoditys.create(registro);
...
async function show(req, res){
	...
	try {
		...
		const registroEncontrado = await db.sequelize.models.commoditys.findByPk(id,{include:relaciones,paranoid: false});
...
async function update(req, res){
	...
	try {
		...
		const registroAEditar = await db.sequelize.models.commoditys.findByPk(id);
		...
		const registrosEncontrados = await db.sequelize.models.commoditys.findAll(whereFind);
		if(registrosEncontrados.length > 0){
			var regExistente = false
			await registrosEncontrados.forEach(registro => {
				if(((registro.descripcion.toLowerCase() == (parametros.descripcion.toLowerCase() != undefined ? parametros.descripcion.toLowerCase() : registroAEditar.descripcion.toLowerCase()))) &&
					 registro.id != id){
						if(!regExistente){
							regExistente = true;
							res.status(400).send({ status: false, msg: "Registro existente"});
		...
		var registro2 = {
			...
			tabla: db.sequelize.models.commoditys.name.toUpperCase(),
			accion: 'EDICION',
			createdAt: moment().tz('America/Mexico_City')
		}
...
async function destroy(req,res){
	...
	try {
		const registroAEliminar = await db.sequelize.models.commoditys.findByPk(id);
		if(registroAEliminar != null){
			...
			for (const modelo of Object.values(db.sequelize.models)) {
				...
				for (const asociacion of Object.values(asociaciones)) {
					if(asociacion.target.name == db.sequelize.models.commoditys.name){
						let where = {}
...
async function restaurar(req,res){
	...
	try {
		const registroARestaurar = await db.sequelize.models.commoditys.findByPk(id,{ paranoid: false });
...
async function showHistoricos(req, res) {
	...
	if(registro.tabla != db.sequelize.models.commoditys.name.toUpperCase()){
		return res.status(400).send({ status: false, msg: "Registro no pertenece al crud commoditys" });
	} 
	let usuario = await db.sequelize.models.usuarios.findByPk(registro.id_usuario_registro);
	reg.id = registro.id;
	reg.usuario_registro = {id: usuario.id, nombre: usuario.nombre};
	...
	try {
		...
		for (let i = 0; i < docs.length; i++) {
			...
			aux = {
				...
				'Categoría': elemento.categoria != null ?  elemento.categoria.descripcion : 'NA',
		...
		const nombreReporte = 'Commodities';
		const namesSheets = [db.sequelize.models.commoditys.name];
